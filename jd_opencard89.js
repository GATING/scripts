...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = ["a8a146949b184828b7339e95de7db5fb"];
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=uniongame2022030101goddess&shareUuid=${encodeURIComponent(
        $.authorCode
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      if ($.openCardList) {
        ...
        for (let i = 0; i < $.openCardList.length; i++) {
          await task(
            ...
            `venderId=${$.activityShopId}&pageId=${
              $.activityId
            }&elementId=入会跳转&pin=${encodeURIComponent($.secretPin)}`,
            1
          );
          await $.wait(500);
        }
      ...
      await task(
        ...
        `activityId=${$.activityId}&pin=${encodeURIComponent(
          $.secretPin
        )}&shareUuid=${$.authorCode}&actorUuid=${$.actorUuid}`,
        1
      );
      await getFirstLZCK();
      await getToken();

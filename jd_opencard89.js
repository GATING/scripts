...
!(async () => {
  ...
  $.shareUuid = "a8a146949b184828b7339e95de7db5fb";
  console.log(
...
async function run() {
  try {
    ...
    if ($.index % 5 == 0) await $.wait(parseInt(Math.random() * 5000, 10));
  } catch (e) {
    console.log(e);
  }
}
  ...
  switch (type) {
    ...
    case "activityContent":
      ...
      body = `activityId=${$.activityId}&pin=${encodeURIComponent(
        ...
        $.nickname
      )}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid}&adsource=null`;
      break;
    case "drawContent":
    ...
    case "addSku":
      ...
      body = `activityId=${$.activityId}&pin=${encodeURIComponent(
        $.Pin
      )}&actorUuid=${$.actorUuid}&taskType=${taskType}`;
      break;

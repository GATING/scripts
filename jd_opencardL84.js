...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = ["971f03c2353644feb66fe3a8617d7bb9"];
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity?activityId=${
        $.activityId
    ...
    if ($.secretPin) {
      ...
      if ($.activityContent) {
        await task(
          ...
          `activityId=${$.activityId}&pin=${encodeURIComponent(
            $.secretPin
          )}&shareUuid=${encodeURIComponent($.authorCode)}&actorUuid=${
            $.actorUuid
          }`
        );
        $.log("\n关注店铺");
        if (!$.activityContent["followShop"].allStatus) {
          await task(
            "shop/league/saveTask",
            `activityId=${$.activityId}&pin=${encodeURIComponent(
              $.secretPin
            )}&actorUuid=${$.actorUuid}&shareUuid=${encodeURIComponent(
              $.authorCode

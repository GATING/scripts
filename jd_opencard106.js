...
!(async () => {
  ...
  authorCodeList = ["d7788ce31fcd4668a505f6fb5b071fb3"];
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/baby/pairysactivity?activityId=${
        $.activityId
      }&shareUuid=${encodeURIComponent(
        $.authorCode
      )}&adsource=undefined/activity/8258555?activityId=dzf14101834564a0b1336646135829&shareuserid4minipg=${encodeURIComponent(
        $.secretPin
      )}&shopid=1000001683&sid=&un_area=`;
      await member();
      await $.wait(500);
      if ($.bean > 0) {
        message += `\n【京东账号${$.index}】${
    ...
    if ($.secretPin) {
      ...
      await taskaccessLog(
        ...
        `venderId=${$.activityShopId}&code=99&pin=${encodeURIComponent(
          ...
          $.activityUrl
        }&subType=app&adSource=undefined/activity/8258555?activityId=dzf14101834564a0b1336646135829`,
        1
      );
      await task(

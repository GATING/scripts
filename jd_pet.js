...
if ($.isNode()) {
  ...
  console.log(
    `友情提示：设置环境变量 DDMC_FORBID_ACCOUNT="pin1,pin2,pin3,..." 控制哪个京东账号不运行此脚本。多个帐号使用,连接。`
  );
  // 根据pin值过滤帐号
...
async function GetShareCode() {
  try {
    ...
    if (
      ...
      $.petInfo = initPetTownRes.result;
      if (
        $.petInfo.userStatus == 0 ||
        $.petInfo.petStatus == 5 ||
        $.petInfo.petStatus == 6 ||
        !$.petInfo.goodsInfo
      ) {
        console.log(
          `【京东账号${$.index}（${$.UserName}）的互助码】\n宠物状态不能被助力，跳过...`
        );
        return;
      }
      console.log(

...
async function composePearlState(type) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(`user/ComposePinPinPearlState`, `__t=${Date.now()}&dwGetType=0`),
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            switch (type) {
              ...
              case 3:
                ...
                if (data.iRet === 0) {
                  console.log(
                    `当前已合成${data.dwCurProgress}颗生鲜，总计获得${data.ddwVirHb}个金豆子`
                  );
                  if (data.strDT) {
  ...
  return new Promise((resolve) => {
    $.get(
      taskUrl(
        ...
        `strBT=${strDT}&strLT=${strLT}&dwIsPP=1`
      ),
      (err, resp, data) => {
        try {
          if (err) {
...
function getPearlDailyReward() {
  return new Promise((resolve) => {
    $.get(
      taskUrl(`user/GetPpPearlDailyReward`, `__t=${Date.now()}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} PpPearlDailyDraw API请求失败，请检查网路重试`
            );
          } else {
            data = JSON.parse(
              data
      ...
      (err, resp, data) => {
        try {
          if (err) {
            ...
            if (data.iRet === 0) {
              console.log(
                `模拟操作中奖：获得${data.ddwAwardHb}个金豆子，总计获得${data.ddwVirHb}个金豆子`
              );
            } else {
      ...
      (err, resp, data) => {
        try {
          if (err) {
            ...
            if (data.iRet === 0) {
              console.log(
                `领取助力奖励成功：获得${data.StagePrizeInfo.ddwAwardHb}个金豆子，总计获得${data.StagePrizeInfo.ddwVirHb}个金豆子`
              );
            } else {
...
function taskUrl(function_path, body = "", dwEnv = 7) {
  ...
  url += `&_ste=1&h5st=${decrypt(Date.now(), "", "", url)}&_=${
    Date.now() + 2
  }&sceneval=2&g_login_type=1&dwIsPP=1&callback=jsonpCBK${String.fromCharCode(
...
function readShareCode() {
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `https://ghproxy.com/https://raw.githubusercontent.com/jiulan/helpRepository/main/json/cfd_hb.json`,
        timeout: 10000,
      },
      (err, resp, data) => {
        try {
...
function getJxToken() {
  ...
  function _0xe18f69(_0x5487a9) {
    ...
    for (
      let _0x497a6a = 0x0;
      _0x1e2686["MNRFu"](_0x497a6a, _0x5487a9);
      _0x497a6a++
    ) {
      _0x2b8bca +=
        _0x3f25a6[
          _0x1e2686["gkPpb"](
            parseInt,
            _0x1e2686["KPODZ"](Math["random"](), _0x3f25a6["length"])
          )
        ];
    }
    return _0x2b8bca;

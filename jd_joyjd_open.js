...
!(async () => {
  ...
  if (!activityIDList) {
    ...
    let data = await getData(
      "https://cdn.jsdelivr.net/gh/KingRan/shareCodes@master/joyjd_open.json"
    );
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      for (let j = 0; j < activityIDArr.length; j++) {
        ...
        console.log(
          `通用活动任务ID：${activityID}，结束时间：${endTime}，活动名称：${taskname}`
        );
...
async function main() {
  ...
  do {
    ...
    for (let i = 0; i < $.dailyTaskList.length; i++) {
      ...
      console.log(
        `groupType:${$.oneTask.groupType},已完成:${$.oneTask.finishCount}次，需要完成：${$.oneTask.taskCount}次`
      );
      $.item = $.oneTask.item;

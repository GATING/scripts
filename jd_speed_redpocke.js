...
async function sign() {
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      body: `functionId=apSignIn_day&body=${escape(
        JSON.stringify(body)
...
function signPrizeDetailList() {
  return new Promise((resolve) => {
    ...
    const options = {
      url: `https://api.m.jd.com`,
      body: `functionId=signPrizeDetailList&body=${escape(
        JSON.stringify(body)
      )}&_t=${+new Date()}&appid=activities_platform`,
      headers: {
        Cookie: cookie,
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0) {
              if (data.data.code === 0) {
                const list = (
                  data.data.prizeDrawBaseVoPageBean.items || []
                ).filter(
                  (vo) => vo["prizeType"] === 4 && vo["prizeStatus"] === 0
                );
                for (let code of list) {
                  console.log(
                    `极速版签到提现，去提现${code["prizeValue"]}现金\n`
                  );
                ...
                console.log(
                  `极速版签到查询奖品：失败:${JSON.stringify(data)}\n`
                );
              }
            } else {
...
function apCashWithDraw(id, poolBaseId, prizeGroupId, prizeBaseId) {
  return new Promise((resolve) => {
    ...
    const options = {
      url: `https://api.m.jd.com`,
      body: `functionId=apCashWithDraw&body=${escape(
        JSON.stringify(body)
      )}&_t=${+new Date()}&appid=activities_platform`,
      headers: {
        Cookie: cookie,
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0) {
              if (data.data.status === "310") {
                ...
                console.log(
                  `极速版签到提现现金：失败:${JSON.stringify(data)}\n`
                );
              }
            } else {

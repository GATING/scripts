...
async function help(sharecode) {
  ...
  res = await api("sign/helpSign", "flag,sceneval,token", {
    flag: 0,
    token: sharecode,
  });
  await $.wait(3000);
...
async function drawUserTask() {
  ...
  if (res.datas) {
    for (let t of res.datas) {
      if (t.state !== 2) tasks.push(t.taskid ? t.taskid : t.taskId);
    }
  } else {
  ...
  console.log(
    `总任务数：${res.datas && res.datas.length}   本次执行任务数: ${
      tasks && tasks.length
    }`
  );
  await $.wait(2000);
  ...
  for (let t of res.data.tasks) {
    if (tasks.includes(t.taskid ? t.taskid : t.taskId)) {
      let sleep =
        (t.param7 ? t.param7 : 2) * 1000 + (Math.random() * 5 + 1) * 1000;
      console.log(`任务名：${t.taskName}    浏览时间：${sleep / 1000} s`);
      res = await api("task/drawUserTask", "sceneval,taskid", {
        taskid: t.taskid ? t.taskid : t.taskId,
      });
      await $.wait(sleep);
      res = await api("task/UserTaskFinish", "sceneval,taskid", {
        taskid: t.taskid ? t.taskid : t.taskId,
      });
  ...
  for (let j = 0; j < surplusTimes && coin >= 10; j++) {
    ...
    if (res) {
      if (res.retCode == 0) {
        console.log(
          "抽奖成功",
          res.data && res.data.prize ? res.data.prize[0].prizename : ""
        );
      } else {
...
async function api(fn, stk, params) {
  ...
  url = await decrypturl(
    `${url}/${fn}?sceneval=2&_stk=active,activedesc,sceneval&_ste=1&_=${Date.now()}&sceneval=2`,
    stk,
    params,
    10012
  );

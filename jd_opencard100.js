...
console.log(
  "脚本部份加密,不喜欢勿跑!!!,10次抽奖机会,3个助力一次抽奖,入口 https://lzkjdz-isv.isvjcloud.com/esteelauder/inviteNew/activityPage?activityId=2204100037674501"
);
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = ["fd83ed6684a8449a8336c4da1b48a5fc"];
      ...
      $.activityUrl = `https://lzkjdz-isv.isvjcloud.com/esteelauder/inviteNew/activityPage?activityId=${
        $.activityId
      }&inviterUuid=${encodeURIComponent(
        $.authorCode
      )}&shareuserid4minipg=${encodeURIComponent($.secretPin)}&sid=&un_area=`;
      await member();
      await $.wait(5000);
      if ($.bean > 0) {
        message += `\n【京东账号${$.index}】${
...
async function member() {
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      if ($.index === 1) {
        await task(
          "inviteNew/activityContent",
          `activityId=${$.activityId}&pin=${encodeURIComponent(
            $.secretPin
          )}&inviterUuid=${encodeURIComponent($.authorCode)}`,
          0,
          1
        );
      } else {
      ...
      await getShopOpenCardInfo(
        { venderId: `${$.activityShopId}`, channel: "401" },
        $.activityShopId
      );
...
function taskUrl(function_id, body, isCommon) {
  return {
    url: isCommon
      ...
      : `https://lzkjdz-isv.isvjcloud.com/esteelauder/${function_id}`,
    headers: {

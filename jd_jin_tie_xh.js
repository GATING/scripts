          ...
          if (data.resultCode === 0) {
            if (data.resultData.code === "000") {
              ...
              for (let i in data.resultData.data.signDetail) {
                if (data.resultData.data.signDetail[i].signed)
                  $.keepSigned += 1;
                if (
                  data.resultData.data.dayId ===
                  data.resultData.data.signDetail[i].id
                ) {
          ...
          if (data.resultCode === 0) {
            if (data.resultData.code === "000") {
              if (data.resultData.data.signSuccess) {
                console.log(
                  `签到成功，获得 0.0${data.resultData.data.rewardAmount}元`
                );
              }
        ...
        if (err) {
          ...
          if (data.resultCode === 0) {
            if (data.resultData.code === "0000") {
              ...
              data.resultData.code === "0001" ||
              data.resultData.code === "0004"
            ) {
              console.log("状态不正确");
            } else {
      ...
      try {
        if (err) {
          ...
          if (data.resultCode === 0) {
            if (data.resultData.code === "000") {
              console.log(
                `\n京东账号${$.index} ${$.nickName || $.UserName} 当前总金贴：${
                  data.resultData.data.availableAmount
                }元`
              );
            } else {
...
function taskUrl_xh(function_id, body, type = "mission") {
  return {
    url: `https://ms.jr.jd.com/gw/generic/${type}/h5/m/${function_id}?reqData=${encodeURIComponent(
      body
    )}`,
    headers: {
      Accept: `*/*`,

...
!(async () => {
  ...
  if (!jd_redrain_half_url) {
    $.log(`尝试使用默认远程url`);
    jd_redrain_half_url =
      "https://raw.githubusercontent.com/Ca11back/scf-experiment/master/json/redrain_half.json";
  }
  ...
  if (!redIds || !redIds.length) {
    $.log(`尝试使用cdn`);
    jd_redrain_half_url =
      "https://raw.fastgit.org/Ca11back/scf-experiment/master/json/redrain_half.json";
        ...
        if (err) {
          ...
          if (data) {
            ...
            await doInteractiveAssignment(
              data.activityInfo.encryptProjectId,
              data.activityInfo.encryptAssignmentId
            );
          }
        }
      } catch (e) {
      ...
      try {
        if (err) {
          ...
          if (data) {
            if (data.subCode == "0") {
              console.log(
                `${data.rewardsInfo.successRewards[3][0].rewardName}个`
              );
            } else {
              console.log(data);
            }

          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.success === true) {
                console.log(
                  data.data.floorInfoList[0].floorData.rarityActNineBox
                    .raffleActKey
                );
                console.log(data.data.floorInfoList[0].token);
                await pg_interact_interface_invoke(
                  data.data.floorInfoList[0].token,
                  data.data.floorInfoList[0].floorData.rarityActNineBox
                    .raffleActKey
                );
              } else {
...
function pg_interact_interface_invoke(floorToken, raffleActKey) {
  return new Promise((resolve) => {
    $.get(
      taskUrl("pg_interact_interface_invoke", {
        v: "13.2",
      ...
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `pg_interact_interface_invoke API请求失败，请检查网路重试`
            );
          } else {
            if (safeGet(data)) {
              data = JSON.parse(data);
...
function taskUrl(functionId, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${functionId}&body=${encodeURIComponent(
      JSON.stringify(body)
    )}&uuid=&appid=vipChannelHome&loginWQBiz=huiyuan&ext=&t=${new Date().getTime()}`,
    headers: {
      Cookie: cookie,
      Host: "api.m.jd.com",
      "user-agent":

...
[task_local]
#内容鉴赏官
15 3,6 * * * https://raw.githubusercontent.com/GATING/scripts/main/jd_connoisseur.js, tag=内容鉴赏官, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "15 3,6 * * *" script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_connoisseur.js,tag=内容鉴赏官

===============Surge=================
内容鉴赏官 = type=cron,cronexp="15 3,6 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_connoisseur.js

============小火箭=========
内容鉴赏官 = type=cron,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_connoisseur.js, cronexpr="15 3,6 * * *", timeout=3600, enable=true
 */
const $ = new Env("内容鉴赏官");
...
let jdWindfggToken = "";
jdWindfggToken = $.isNode()
  ? process.env.WindfggToken
    ? process.env.WindfggToken
    : `${jdWindfggToken}`
  : $.getdata("WindfggToken")
  ? $.getdata("WindfggToken")
  : `${jdWindfggToken}`;
        ...
        if (err) {
          ...
          if (data) {
            data =
              data &&
              data.match(
                /window\.performance.mark\(e\)}}\((.*)\);<\/script>/
              )[1];
            ...
            for (let key of Object.keys(data.codeFloors)) {
              ...
              if (
                vo.boardParams &&
                vo.boardParams.taskCode === "2CCbSBbVWkFZzRDngs4F6q3YZ62o"
              ) {
                agid = [];
                ...
                await getTaskInfo(
                  "11",
                  vo.boardParams.projectCode,
                  vo.boardParams.taskCode
                );
              } else if (
                vo.boardParams &&
                vo.boardParams.taskCode === "4JHmm8nEpyuKgc3z9wkGArXDtEdh"
              ) {
                agid = [];
                ...
                vo.boardParams &&
                vo.boardParams.taskCode === "2PbAu1BAT79RxrM5V7c2VAPUQDSd"
              ) {
                agid = [];
                agid.push(vo.materialParams.advIdKOC[0].advGrpId);
                agid.push(vo.materialParams.advIdVideo[0].advGrpId);
                ...
                vo.boardParams &&
                (vo.boardParams.taskCode === "485y3NEBCKGJg6L4brNg6PHhuM9d" ||
                  vo.boardParams.taskCode === "2bpKT3LMaEjaGyVQRr2dR8zzc9UU")
              ) {
                console.log(`去做【${vo.boardParams.titleText}】`);
                await getTaskInfo(
                  "9",
                ...
                vo.boardParams &&
                (vo.boardParams.taskCode === "3dw9N5yB18RaN9T1p5dKHLrWrsX" ||
                  vo.boardParams.taskCode === "CtXTxzkh4ExFCrGf8si3ePxGnPy" ||
                  vo.boardParams.taskCode === "Hys8nCmAaqKmv1G3Y3a5LJEk36Y" ||
                  vo.boardParams.taskCode === "2wPBGptSUXNs3fxqgAtrV5MwkYEa")
              ) {
                await getTaskInfo(
                  "1",
                  vo.boardParams.projectCode,
...
async function getTaskInfo(
  ...
  if (assignmentId === $.taskCode)
    (body["itemId"] = itemId), (body["helpType"] = helpType);
        ...
        if (err) {
          ...
          if (data) {
            ...
            if (assignmentId === "2CCbSBbVWkFZzRDngs4F6q3YZ62o") {
              if (data.code === "0" && data.data) {
                if (data.data[0]) {
                  if (data.data[0].status !== "2") {
                    let num = 0;
                    for (
                      let i = data.data[0].viewedNum;
                      i < data.data[0].viewNum;
                      i++
                    ) {
                      let vo = data.data[0].videoDtoPageResult.list[num];
                      await interactive_done(
                        type,
                        data.data[0].projectId,
                        data.data[0].assignmentId,
                        vo.articleDto.jump.id,
                        1
                      );
                    ...
                    num = 0;
                    for (
                      let i = data.data[0].evaluatedNum;
                      i < data.data[0].evaluateNum;
                      i++
                    ) {
                      let vo = data.data[0].videoDtoPageResult.list[num];
                      await interactive_done(
              ...
              if (data.code === "0" && data.data) {
                if (data.data[0]) {
                  if (data.data[0].status !== "2") {
                    ...
                    let num = 0;
                    for (
                      let i = data.data[0].watchAlreadyCount;
                      i < data.data[0].watchTotalCount;
                      i++
                    ) {
                    ...
                    num = 0;
                    for (
                      let i = data.data[0].likeAlreadyCount;
                      i < data.data[0].likeTotalCount;
                      i++
                    ) {
                      let vo = res.data.cardList[num];
                      await interactive_done(
              ...
              assignmentId === "2wPBGptSUXNs3fxqgAtrV5MwkYEa" ||
              assignmentId === "u4eyHS91t3fV6HRyBCg9k5NTUid" ||
              assignmentId === "4WHSXEqKZeGQZeP9SvqxePQpBkpS" ||
              assignmentId === "4PCdWdiKNwRw1PmLaFzJmTqRBq4v" ||
              assignmentId === "4JcMwRmGJUXptBYzAfUDkKTtgeUs" ||
              assignmentId === "4ZmB6jqmJjRWPWxjuq22Uf17CuUQ" ||
              assignmentId === "QGPPJyQPhSBJ57QcU8PdMwWwwCR" ||
              assignmentId === "tBLY4YL4LkBwWj9KKq9BevHHvcP" ||
              ...
              assignmentId === "485y3NEBCKGJg6L4brNg6PHhuM9d" ||
              assignmentId === "2bpKT3LMaEjaGyVQRr2dR8zzc9UU"
            ) {
              if (data.code === "0" && data.data) {
                if (data.data[0].status !== "2") {
                  await sign_interactive_done(
                    type,
                    data.data[0].projectId,
                    data.data[0].assignmentId
                  );
              ...
              if (helpType === "1") {
                if (data.code === "0" && data.data) {
                  if (data.data[0].status !== "2") {
                    console.log(
                      `【京东账号${$.index}（${$.UserName}）的内容鉴赏官好友互助码】${data.data[0].itemId}`
                    );
                    $.shareCodes.push({
                      use: $.UserName,
    ...
    $.post(taskPostUrl(functionId, sign), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(
            `${$.name} sign_interactive_done API请求失败，请检查网路重试`
          );
        } else {
          if (data) {
            data = JSON.parse(data);
      ...
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} interactive_reward API请求失败，请检查网路重试`
            );
          } else {
            if (data) {
              data = JSON.parse(data);
      ...
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} interactive_accept API请求失败，请检查网路重试`
            );
          } else {
            if (data) {
              data = JSON.parse(data);
...
async function qryViewkitCallbackResult(
  encryptProjectId,
  encryptAssignmentId,
  itemId
) {
    ...
    $.post(taskPostUrl(functionId, sign), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(
            `${$.name} qryViewkitCallbackResult API请求失败，请检查网路重试`
          );
        } else {
          if (data) {
            data = JSON.parse(data);
...
async function getshareCode() {
  ...
  let options = {
    ...
    body: `body=${encodeURIComponent(
      body
    )}&screen=1242*2208&sid=${sid}&uuid=${randomString(
      40
    )}&area=&osVersion=15.0.1&d_model=iphone11,2`,
    headers: {
      Host: "api.m.jd.com",
      Accept: "*/*",
            ...
            for (let key of Object.keys(data.floorList)) {
              ...
              if (
                ...
                (vo.boardParams.taskCode === "2gWnJADG8JXMpp1WXiNHgSy4xUSv" ||
                  vo.boardParams.taskCode === "2wPBGptSUXNs3fxqgAtrV5MwkYEa" ||
                  vo.boardParams.taskCode === "u4eyHS91t3fV6HRyBCg9k5NTUid" ||
                  vo.boardParams.taskCode === "Hys8nCmAaqKmv1G3Y3a5LJEk36Y" ||
                  vo.boardParams.taskCode === "4WHSXEqKZeGQZeP9SvqxePQpBkpS" ||
                  vo.boardParams.taskCode === "4PCdWdiKNwRw1PmLaFzJmTqRBq4v" ||
                  vo.boardParams.taskCode === "4JcMwRmGJUXptBYzAfUDkKTtgeUs" ||
                  vo.boardParams.taskCode === "4ZmB6jqmJjRWPWxjuq22Uf17CuUQ" ||
                  vo.boardParams.taskCode === "QGPPJyQPhSBJ57QcU8PdMwWwwCR" ||
                  vo.boardParams.taskCode === "tBLY4YL4LkBwWj9KKq9BevHHvcP" ||
                  vo.boardParams.taskCode === "4UFHr2rSLyS912riDWih6B8gMXkf" ||
                  vo.boardParams.taskCode === "3dw9N5yB18RaN9T1p5dKHLrWrsX")
              ) {
                await getTaskInfo(
                  "1",
                ...
                vo.boardParams &&
                vo.boardParams.taskCode === "3PX8SPeYoQMgo1aJBZYVkeC7QzD3"
              ) {
...
function aha_card_list(type, projectId, assignmentId) {
  return new Promise((resolve) => {
    $.post(
      taskUrl("browse_aha_task/aha_card_list", {
        type: type,
        projectId: projectId,
        assignmentId: assignmentId,
        agid: agid,
    ...
    $.post(url, async (err, resp, data) => {
      try {
        ...
        if (data && data.code == 200) {
          ...
          console.log(
            "连接Windfgg服务成功，当前Token使用次数为" + lnrequesttimes
          );
          if (data.data) strsign = data.data || "";

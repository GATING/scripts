...
[task_local]
#京喜财富岛互助
33 0,14 * * * https://raw.githubusercontent.com/GATING/scripts/main/jd_cfd_help.js, tag=京喜财富岛互助, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "33 0,14 * * *" script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_cfd_help.js,tag=京喜财富岛互助

===============Surge=================
京喜财富岛互助 = type=cron,cronexp="33 0,14 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_cfd_help.js

============小火箭=========
京喜财富岛互助 = type=cron,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_cfd_help.js, cronexpr="33 0,14 * * *", timeout=3600, enable=true
...
!(async () => {
  ...
  let res = await getAuthorShareCode(
    "https://cdn.jsdelivr.net/gh/KingRan/shareCodes@master/cfd.json"
  );
  $.strMyShareIds = [...(res || [])];
              ...
              if (err) {
                ...
                if (msg.indexOf("活动太火爆了") !== -1) {
                  ...
                  if (JSON.parse(prizeInfo).dwPrizeType == 4) {
                    str =
                      msg + prizeInfo
                        ? `获得红包 ¥ ${JSON.parse(prizeInfo).strPrizeName}`
                        : "";
                  } else {
                    str =
                      msg + prizeInfo
      ...
      case 0: //日常任务
        $.get(
          taskListUrl(
            ...
            `taskId=0&showAreaTaskFlag=${$.showPp}`
          ),
          async (err, resp, data) => {
            try {
              if (err) {

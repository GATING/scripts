...
async function main() {
  ...
  if ($.taskList) {
    for (const vo of $.taskList) {
      if (
        vo.ext.extraType !== "brandMemberList" &&
        vo.ext.extraType !== "assistTaskDetail"
      ) {
...
function doInteractiveAssignment(
  projectId,
  encryptAssignmentId,
  itemId,
  actionType
  ...
  let body = {
    ...
    ext: {},
    extParam: {
      businessData: { random: 25500725 },
      signStr: timestamp + "~1hj9fq9",
      sceneid: "XMFhPageh5",
    },
  };
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("doInteractiveAssignment", body),
      async (err, resp, data) => {
        //$.log(data)
        try {
          if (err) {
...
function taskPostUrl(function_id, body) {
  return {
    url: `https://api.m.jd.com/?functionId=${function_id}&body=${escape(
      JSON.stringify(body)
    )}&appid=content_ecology&client=wh5&clientVersion=1.0.0`,
    headers: {
      Host: "api.m.jd.com",
      ...
      "User-Agent": UA,
      Referer:
        "https://h5.m.jd.com/babelDiy/Zeus/2bf3XEEyWG11pQzPGkKpKX2GxJz2/index.html",
      Cookie: cookie,
    },
  };
}

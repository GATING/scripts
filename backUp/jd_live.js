          ...
          if (safeGet(data)) {
            ...
            if (data.data.starLiveList) {
              for (let key of Object.keys(data.data.starLiveList)) {
                ...
                if (vo.state !== 3) {
                  let authorId = (await getauthorId(vo.extra.liveId)).data
                    .author.authorId;
...
function viewTask() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("liveChannelReportDataV912", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function awardTask(type = "shareTask", liveId = "2942545") {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl("getChannelTaskRewardToM", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function sign() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("getChannelTaskRewardToM", { type: "signTask", itemId: "1" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&appid=h5-live&body=${encodeURIComponent(
      JSON.stringify(body)
    )}&v=${
      new Date().getTime() +
      new Date().getTimezoneOffset() * 60 * 1000 +
    ...
    headers: {
      ...
      "Accept-Encoding": "gzip, deflate, br",
      Referer:
        "https://cfe.m.jd.com/privatedomain/live-boborock/20210809/index.html",

...
async function main(ck) {
  let usName = decodeURIComponent(
    ck.match(/pt_pin=([^; ]+)(?=;?)/) && ck.match(/pt_pin=([^; ]+)(?=;?)/)[1]
  );
  let UA = `jdapp;iPhone;10.0.8;14.6;${randomWord(
    false,
    40,
    40
  ...
  let mainInfo = await takePost(
    '{"apiMapping":"/api/index/indexInfo"}',
    ck,
    UA
  );
  ...
  for (let i = 0; i < 60 && showFlag; i++) {
    showFlag = false;
    let pageNum = i + 1;
    let pageInfo = await takePost(
      `{"pageNum":${pageNum},"apiMapping":"/api/record/prizeRecord"}`,
      ck,
      UA
    );

...
[task_local]
#萌虎摇摇乐
0 0,12,18 * * *  https://raw.githubusercontent.com/KingRan/JDJB/main/jd_teamFLP.js, tag=组队分豆-飞利浦, enabled=true
*/
const name = "萌虎摇摇乐";
let UA = process.env.JD_USER_AGENT
    ...
    try {
      ...
      for (let tab of res.data) {
        ...
        res = await api({
          ...
          apiMapping: "/api/task/brand/getTaskList",
        });
          ...
          for (let i = t.finishNum; i < t.totalNum; i++) {
            ...
            if (res) {
              ...
              res = await api({
                taskGroupId: taskGroupId,
                taskId: res.taskId,
                taskItemId: res.taskItemId,
                apiMapping: "/api/task/brand/doTask",
              });
              await wait(sleep);
              if (res.data.taskType === "BROWSE_TASK") {
                res = await api({
                  ...
                  taskItemId: res.data.taskItemId,
                  timestamp: res.data.timeStamp,
                  apiMapping: "/api/task/brand/getReward",
                });
                console.log(
                  "任务完成，积分：",
                  res.data.integral,
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let code of shareCodes) {
      ...
      try {
        const res = await api({
          shareId: code,
          apiMapping: "/api/task/support/doSupport",
        });
        if (res.data.status === 1) {
          !res.data.supporterPrize
            ...
            : console.log(
                "助力成功，京豆：",
                res.data.supporterPrize.beans,
                "，积分：",
                res.data.supporterPrize.score
              );

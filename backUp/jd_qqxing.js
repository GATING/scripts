...
function getActCk() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("/dingzhi/qqxing/pasture/activity", `activityId=90121061401`),
      (err, resp, data) => {
        updateCookie(resp);
        // console.log(resp)
        try {
...
function adlog() {
  let config = taskPostUrl(
    ...
    `venderId=1000361242&code=99&pin=${encodeURIComponent(
      $.pin
    )}&activityId=90121061401&pageUrl=https%3A%2F%2Flzdz-isv.isvjcloud.com%2Fdingzhi%2Fqqxing%2Fpasture%2Factivity%3FactivityId%3D90121061401%26lng%3D107.146945%26lat%3D33.255267%26sid%3Dcad74d1c843bd47422ae20cadf6fe5aw%26un_area%3D27_2442_2444_31912&subType=app&adSource=`
  );
  //   console.log(config)
  return new Promise((resolve) => {
    $.post(config, async (err, resp, data) => {
...
function getUid() {
  return new Promise((resolve) => {
    let body = `activityId=90121061401&pin=${encodeURIComponent(
      $.pin
    )}&pinImg=${$.pinImg}&nick=${encodeURIComponent(
      $.nick
    )}&cjyxPin=&cjhyPin=&shareUuid=${$.shareuuid}`;
    $.post(
      taskPostUrl("/dingzhi/qqxing/pasture/activityContent", body),
      async (err, resp, data) => {
        updateCookie(resp);
        try {
          if (err) {
...
function getinfo() {
  let config = taskPostUrl(
    "/dingzhi/qqxing/pasture/myInfo",
    `activityId=90121061401&pin=${encodeURIComponent($.pin)}&pinImg=${
      $.pinImg
    ...
    $.post(config, async (err, resp, data) => {
      ...
      try {
        if (err) {
          ...
          data = JSON.parse(data);
          if (data.result) {
            $.taskList = data.data.task.filter(
              (x) => (x.maxNeed == 1 && x.curNum == 0) || x.taskid == "interact"
            );
            $.taskList2 = data.data.task.filter(
              (x) => x.maxNeed != 1 && x.type == 0
            );
            ...
            let helpinfo = data.data.task.filter(
              (x) => x.taskid == "share2help"
            )[0];
            if (helpinfo) {
            ...
            $.cow = `当前🐮🐮成长值：${$.score}  饲料：${
              $.food.totalNum - $.food.useNum
            }  抽奖次数：${$.draw.totalNum - $.draw.useNum}  签到天数：${
              $.sign.totalNum
            }`;
...
function getproduct() {
  return new Promise((resolve) => {
    let body = `type=4&activityId=90121061401&pin=${encodeURIComponent(
      $.pin
    )}&actorUuid=${$.uuid}&userUuid=${$.uuid}`;
    $.post(
      taskPostUrl("/dingzhi/qqxing/pasture/getproduct", body),
      async (err, resp, data) => {
        updateCookie(resp);
        try {
          if (err) {
...
function writePersonInfo(vid) {
  return new Promise((resolve) => {
    let body = `jdActivityId=1404370&pin=${encodeURIComponent(
      $.pin
    )}&actionType=5&venderId=${vid}&activityId=90121061401`;

    $.post(
      taskPostUrl("/interaction/write/writePersonInfo", body),
      async (err, resp, data) => {
        updateCookie(resp);
        try {
          if (err) {
...
function exchange(id) {
  return new Promise((resolve) => {
    let body = `pid=${id}&activityId=90121061401&pin=${encodeURIComponent(
      $.pin
    )}&actorUuid=&userUuid=`;
    $.post(
      taskPostUrl("/dingzhi/qqxing/pasture/exchange?_", body),
      async (err, resp, data) => {
        updateCookie(resp);
        try {
          if (err) {
...
function dotask(taskId, params) {
  let config = taskPostUrl(
    "/dingzhi/qqxing/pasture/doTask",
    `taskId=${taskId}&${
      params ? "param=" + params + "&" : ""
    }activityId=90121061401&pin=${encodeURIComponent($.pin)}&actorUuid=${
      $.uuid
    }&userUuid=${$.shareuuid}`
  );
  //     console.log(config)
...
function draw() {
  let config = taskPostUrl(
    "/dingzhi/qqxing/pasture/luckydraw",
    `activityId=90121061401&pin=${encodeURIComponent(
      $.pin
    )}&actorUuid=&userUuid=`
  );
  //  console.log(config)

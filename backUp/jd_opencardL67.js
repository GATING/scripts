...
async function run() {
  try {
    ...
    let config = [
      {
        configCode: "105d40dc86794724b1ace8019fa8e1b2",
        configName: "情人节活动",
      },
    ];
    for (let i in config) {
      ...
      if ($.task.showOrder) {
        console.log(
          `\n[${item.configName}] ${
            ($.task.showOrder == 2 && "日常任务") ||
            ($.task.showOrder == 1 && "开卡") ||
            "未知活动类型"
          } ${($.taskInfo.rewardStatus == 2 && "全部完成") || ""}`
        );
        ...
        for (let i = 0; i < $.taskList.length; i++) {
          ...
          if ($.task.showOrder == 1) {
            ...
            console.log(
              `${$.oneTask.cardName} ${
                0 == $.oneTask.result
                  ? "开卡得" + $.oneTask.rewardQuantity + "京豆"
                  : 1 == $.oneTask.result
                  ? "领取" + $.oneTask.rewardQuantity + "京豆"
                  : 3 == $.oneTask.result
                  ? "其他渠道入会"
                  : "已入会"
              }`
            ...
            if ($.oneTask.result == 0)
              await statistic(
                `{"activityType":"module_task","groupType":7,"configCode":"${item.configCode}","itemId":${$.oneTask.cardId}}`
              );
            if ($.oneTask.result == 0) await join($.oneTask.venderId);
            await $.wait(parseInt(Math.random() * 1000 + 500, 10));
            if ($.oneTask.result == 1 || $.oneTask.result == 0)
              await getReward(
                `{"configCode":"${item.configCode}","groupType":7,"itemId":${$.oneTask.cardId},"eid":"${$.eid}","fp":"${$.fp}"}`
              );
            ...
            let status = `${
              ($.oneTask.finishCount >= $.oneTask.taskCount && "已完成") ||
              "去" +
                (1 == $.oneTask.groupType
                  ? "关注"
                  : 2 == $.oneTask.groupType
            ...
            if ($.oneTask.finishCount < $.oneTask.taskCount) {
              await doTask(
                `{"configCode":"${item.configCode}","groupType":${$.oneTask.groupType},"itemId":"${$.oneTask.item.itemId}","eid":"${$.eid}","fp":"${$.fp}"}`
              );
              ...
              for (n = 2; c-- && !$.outActivity; n++) {
                ...
                if ($.oneTasks) {
                  ...
                  if (
                    $.oneTasks.item.itemId != $.oneTask.item.itemId &&
                    $.oneTasks.finishCount < $.oneTasks.taskCount
                  )
                    await doTask(
                      `{"configCode":"${item.configCode}","groupType":${$.oneTasks.groupType},"itemId":"${$.oneTasks.item.itemId}","eid":"${$.eid}","fp":"${$.fp}"}`
                    );
        ...
        if ($.task.showOrder == 2) {
          if ($.doTask) {
            ...
            for (m = 1; q--; m++) {
              if ($.taskInfo == "")
                await getActivity(item.configCode, item.configName, -1);
              if ($.taskInfo) break;
            }
          }
          if ($.taskInfo.rewardStatus == 1)
            await getReward(
              `{"configCode":"${item.configCode}","groupType":5,"itemId":1,"eid":"${$.eid}","fp":"${$.fp}"}`,
              1
            );
        }
      }
            ...
            if (typeof res == "object") {
              if (res.success == true && res.data.pass == true) {
                if (flag == 0) {
                  ...
                  $.taskInfo =
                    res.data.moduleBaseInfo || res.data.moduleBaseInfo || [];
                } else if (flag == -1) {
                  $.taskInfo =
                    res.data.moduleBaseInfo || res.data.moduleBaseInfo || {};
          ...
          if (err) {
            ...
            if (typeof res == "object") {
              if (res.success == true) {
                console.log(
                  `领奖成功:${
                    (flag == 1 && $.taskInfo.rewardFinish) ||
                    $.oneTask.rewardQuantity
                  }京豆`
                );
                $.bean += Number($.oneTask.rewardQuantity);
              } else {

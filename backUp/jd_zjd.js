...
!(async () => {
  ...
  console.log(
    `\n\n内部互助 【赚京豆-瓜分京豆(微信小程序)-瓜分京豆】活动(内部账号互助(需内部cookie数量大于${
      $.assistNum || 4
    }个))\n`
  );

  let Earn_NUM = process.env.Earn_NUM || cookiesArr.length;
  for (let i = 0; i < Earn_NUM; i++) {
    $.canHelp = true;
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $.UserName = decodeURIComponent(
      ...
      if (cookiesArr.length > $.assistNum) {
        ...
        for (let j = 0; j < $.tuanList.length; ++j) {
          console.log(
            `账号 ${$.UserName} 开始给 【${$.tuanList[j]["assistedPinEncrypted"]}】助力`
          );
          await helpFriendTuan(
            $.tuanList[j]["activityIdEncrypted"],
            $.tuanList[j]["assistStartRecordId"],
            $.tuanList[j]["assistedPinEncrypted"]
          );
          if (!$.canHelp) break;
          await $.wait(3000);
        }
...
async function distributeBeanActivity() {
  try {
    ...
    if (
      !$.tuan &&
      ($.assistStatus === 3 || $.assistStatus === 2 || $.assistStatus === 0) &&
      $.canStartNewAssist
    ) {
      console.log(`准备再次开团`);
      await openTuan();
    ...
    if (
      $.tuan &&
      $.tuan.hasOwnProperty("assistedPinEncrypted") &&
      $.assistStatus !== 3
    ) {
      $.tuanList.push($.tuan);
    }
...
function pg_interact_interface_invoke(floorToken) {
  ...
  const options = {
    url: `${JD_API_HOST}?functionId=pg_interact_interface_invoke&body=${escape(
      JSON.stringify(body)
    )}&appid=swat_miniprogram&fromType=wxapp&timestamp=${
      new Date().getTime() +
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data["success"]) {
              console.log(
                `【做任务 天天领京豆】${data["data"]["rewardBeanAmount"]}京豆领取成功`
              );
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data["success"]) {
              console.log(
                `活动开启成功，初始：${
                  data.data && data.data["activityBeanInitAmount"]
                }京豆`
              );
              $.vvipFlag = true;
            } else {
...
function helpFriendTuan(
  ...
  return new Promise(async (resolve) => {
    let body = {
      activityIdEncrypted: activityIdEncrypted,
      assistStartRecordId: assistStartRecordId,
      assistedPinEncrypted: assistedPinEncrypted,
    ...
    $.post(
      taskTuanUrl("vvipclub_distributeBean_assist", body, h5st),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.success) {
                ...
                if (data.resultCode === "9200008")
                  ...
                  $.canHelp = false;
                } else if (data.resultCode === "9000013") {
                  console.log("助力结果：活动火爆，跳出\n");
                  $.canHelp = false;
...
function getUserTuanInfo() {
  return new Promise(async (resolve) => {
    ...
    $.post(
      taskTuanUrl("distributeBeanActivityInfo", body, h5st),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data["success"]) {
                $.log(
                  `\n\n当前【赚京豆-瓜分京豆(微信小程序)-瓜分京豆】能否再次开团: ${
                    data.data.canStartNewAssist ? "可以" : "否"
                  }`
                );
                console.log(`assistStatus ${data.data.assistStatus}`);
                if (
                  data.data.assistStatus === 1 &&
                  !data.data.canStartNewAssist
                ) {
                  console.log(`已开团(未达上限)，但团成员人未满\n\n`);
                } else if (
                  data.data.assistStatus === 3 &&
                  data.data.canStartNewAssist
                ) {
                ...
                $.tuan = true; //活动火爆
                console.log(
                  `赚京豆-瓜分京豆(微信小程序)-瓜分京豆】获取【活动信息失败 ${JSON.stringify(
                    data
                  )}\n`
                );
              }
...
function openTuan() {
  return new Promise(async (resolve) => {
    ...
    let h5st = h5stSign(body) || "undefined";
    $.post(
      taskTuanUrl("vvipclub_distributeBean_startAssist", body, h5st),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&body=${escape(
      JSON.stringify(body)
    )}&appid=swat_miniprogram&&h5st=${h5st}&osVersion=5.0.0&clientVersion=3.1.3&fromType=wxapp&timestamp=${
      new Date().getTime() +
      new Date().getTimezoneOffset() * 60 * 1000 +
      8 * 60 * 60 * 1000
    }`,
...
function taskTuanUrl(function_id, body = {}, h5st) {
  return {
    ...
    body: `body=${escape(
      JSON.stringify(body)
    )}&appid=swat_miniprogram&h5st=${h5st}&uuid=61673901346831643128800601&client=tjj_m&screen=1920*1080&osVersion=5.0.0&networkType=wifi&sdkName=orderDetail&sdkVersion=1.0.0&clientVersion=3.1.3&area=11`,
    headers: {
      Accept: "*/*",
      "Accept-Encoding": "gzip, deflate, br",
      "Accept-Language": "zh-cn",
      ...
      Host: "api.m.jd.com",
      Referer:
        "https://servicewechat.com/wxa5bf5ee667d91626/182/page-frame.html",
      Cookie: cookie,
      "User-Agent":
...
var _0xode = "jsjiami.com.v6",
  ...
  _0x3e5c = [
    ...
    "w44ORMKkw7k=",
    "VMKvRzjDtw==",
    "w4tJQ3hgBh3DvwrDtAM8w5kbI2XCpkwOJDbDgcKMEsKbB1/Cu8KOXMKLw5REDHV5wokLHz/Ci31Pwr8=",
    "NsOkVcKDLA==",
    "JGvDrC0swqhpenTDu8OswrLDik7ClhxQw45/w7/DjWV3",
    "wqQJLcKUW8KoQ8KcYzNVFcOVFFdKw5DClMORw5fDnMOuwq0=",
    "w7rCsMOnMng=",
    "woMSKHvDnhzClXvDmhLChsKXwoxYP1jDhE/DgmjCjMOZw7Q=",
    "w5ocfH/DnB/CtXDClgrCnsKDwowSOULDikHChmzDh8KOwrPCl8OvfcKHeMKwJg==",
    "w7Qgw44=",
    "CcKKQkQ6",
    "E1rDncKNwoc=",
    "wqrCjR/ClQ==",
    ...
    "w6k7w4tLw50r",
    "wq0RKcKMScOwCcOH",
    "wqsLc8K8QWHDjhQQwqJvwpNFwpfCpMK3BcO2w7h8UnxXVGoOI0p3wrZAN8K5wpc3IsObwp3DoWLCuk1owotHwrocw6XDmS8UXg==",
    "wrLDqUo=",
    "LWrCuws=",
    "w7fCkwHDk3rCtw3CmA==",
    "TFPDggvCrQ==",
    "wpPCg8K5wpNdfxfChjLCpg==",
    "wr3CusKvwq54",
    "C0rDt2XDkg==",
    "KFJkKwc=",
    "w4JkcH3CvsK7w4BcKsOp",
    "OcKiacKyDA==",
    "wppFwoHCpjM=",
    "EcOBecKiwpUT",
    "wqAYN8KDXMO6",
    ...
    "IsOZdMKpGg==",
    "wpRhw6o=",
    "KcOHIMOsPkLCisOpwq7DnkvDqz0=",
    "dg0uV1M=",
    "en7DuDE5wqzDgsOKWMK5VjPCscOUw6Q=",
    "wo7Dl8K1wrowMGgKJ8K6wpHCtQvCnXxUJg==",
    "w7xadMOvHA==",
    "wqTCvSs=",
    "C8KpVEUR",
    "IElGPRk=",
    ...
    "w70qw41mw5I4TQ==",
    "wpDDi8K4wosSCH4d",
    "woTDmcOyw7Z3w7DCqsOR",
    "wp9VKlPDhQHCiWDDkUM=",
    "wr7CnDPDr3HDvlgcwoXDow==",
    "w70qw41mw5I1",
    "wqcxfMOZwro=",
    "AsO6QcKjEQ==",
    "w7PDqG9WI8ODbsK2XGDCiV/DtsKSwq4=",
    "eHrDuikhwpDDlQ==",
    "ehAgMsKzw5Q=",
    "JMOMcMKGC2pYQXEaew==",
    "FMKLVkcqw5U=",
    ...
    "wrXCocK6wqxD",
    "woM6ER/Dmw==",
    "WsKTIsO+w4xJIcK0csKXAGPDksOecMKG",
    "AUDDqcK2wokywpR/HsOfw6nDgCAQL04=",
    "FnrDvMKaZwvDssKSwp4Kw43DijrCqCAAAg==",
    "w4rDt8K4w6RxOj05aEXDi8OyJsKUdcOlwp3CuUHCi8Oew5fCocOm",
    "MwlCwr/Cr0xKw79xw68=",
    "AMKNUGUu",
    "jsjiami.rAcoym.vk6qlwTCTYSrWdy==",
  ];
if (
  ((function (_0x4aa0ba, _0x6651ee, _0x213183) {
    function _0x3f40da(
      _0x66cbe6,
      _0xa8d18c,
      _0x11b699,
      ...
      if (_0xa8d18c < _0x66cbe6) {
        while (--_0x66cbe6) {
          ...
          if (
            _0xa8d18c === _0x66cbe6 &&
            _0x18b84b === "‮" &&
            _0x18b84b["length"] === 0x1
          ) {
            (_0xa8d18c = _0x11a38d), (_0x11b699 = _0x4aa0ba[_0x57b1dc + "p"]());
          } else if (
            _0xa8d18c &&
            _0x11b699["replace"](/[rAykqlwTCTYSrWdy=]/g, "") === _0xa8d18c
          ) {
...
function _0x5722(_0x583609, _0xdaa79d) {
  ...
  if (_0x5722["fwnILs"] === undefined) {
    (function () {
      ...
      _0x2e1e8e["atob"] ||
        (_0x2e1e8e["atob"] = function (_0x12157a) {
          ...
          for (
            var _0x17f039 = 0x0,
              ...
              _0x588938 = 0x0,
              _0x10831c = "";
            (_0x38a27d = _0x357a6d["charAt"](_0x588938++));
            ~_0x38a27d &&
            ((_0x7aba02 =
              _0x17f039 % 0x4 ? _0x7aba02 * 0x40 + _0x38a27d : _0x38a27d),
            _0x17f039++ % 0x4)
              ? (_0x10831c += String["fromCharCode"](
                  0xff & (_0x7aba02 >> ((-0x2 * _0x17f039) & 0x6))
                ))
              : 0x0
          ) {
    ...
    function _0x38066e(_0x5f582d, _0xdaa79d) {
      ...
      for (
        var _0x360e57 = 0x0, _0x475e55 = _0x5f582d["length"];
        _0x360e57 < _0x475e55;
        _0x360e57++
      ) {
        _0x180c10 +=
          "%" +
          ("00" + _0x5f582d["charCodeAt"](_0x360e57)["toString"](0x10))[
            "slice"
          ](-0x2);
      }
      ...
      for (_0x22963d = 0x0; _0x22963d < 0x100; _0x22963d++) {
        _0x5609f9 =
          (_0x5609f9 +
            _0x3f541e[_0x22963d] +
            _0xdaa79d["charCodeAt"](_0x22963d % _0xdaa79d["length"])) %
          0x100;
      ...
      for (var _0x330cd5 = 0x0; _0x330cd5 < _0x5f582d["length"]; _0x330cd5++) {
        ...
        _0x3f541e[_0x22963d] = _0x3f541e[_0x5609f9];
        _0x3f541e[_0x5609f9] = _0x30716b;
        _0x28413e += String["fromCharCode"](
          _0x5f582d["charCodeAt"](_0x330cd5) ^
            _0x3f541e[(_0x3f541e[_0x22963d] + _0x3f541e[_0x5609f9]) % 0x100]
        );
      }
      return _0x28413e;
    }
...
async function requestAlgo() {
  ...
  do {
    ss =
      _0x236f59["HAWAS"](getRandomIDPro, {
        size: 0x1,
        customDict: _0x3cf7b2,
      }) + "";
    if (
      _0x236f59[_0x5722("‫7", "9I9J")](
        _0x4c279a[_0x5722("‫8", "9I9J")](ss),
        -0x1
      )
    )
      _0x4c279a += ss;
  } while (
    _0x236f59[_0x5722("‫9", "Y^Z7")](_0x4c279a[_0x5722("‮a", "RRac")], 0x3)
  );
  for (let _0x48780e of _0x4c279a[_0x5722("‮b", "yqlT")]())
    _0x32e98d = _0x32e98d[_0x5722("‫c", "WfFI")](_0x48780e, "");
  $["fp"] =
    _0x236f59[_0x5722("‮d", "#FOB")](
      _0x236f59[_0x5722("‮e", "g$*r")](
        getRandomIDPro({
          size: _0x54cb36,
    ...
    _0x236f59["HAWAS"](getRandomIDPro, {
      size: _0x236f59[_0x5722("‮f", "TZxy")](
        _0x236f59["WtFpg"](0xe, _0x236f59["quvkT"](_0x54cb36, 0x3)),
        0x1
      ),
      customDict: _0x32e98d,
    }) +
      ...
      $[_0x5722("‮1b", "b17P")](
        ...
        (_0x2c2c84, _0x1b3374, _0x41f319) => {
          try {
            ...
            $[_0x5722("‮1d", "fP)@")] = new Function(
              _0x5722("‫1e", "WfFI") + result[_0x5722("‫1f", "O*W[")]
            )();
          } catch (_0x23f40a) {
            if (
              _0x236f59["fqadZ"](
                _0x236f59["DbgUc"],
                _0x236f59[_0x5722("‮20", "tYT]")]
              )
            ) {
...
function getRandomIDPro() {
  ...
  var _0x5f0a7a,
    ...
    _0x2b28ff = _0x5ce014["BCJdQ"](
      ...
      ? 0xa
      : _0x58ef1d,
    _0x58ef1d = _0x5ce014["oyejR"](
      void 0x0,
      (_0x58ef1d = _0x484f6f[_0x5722("‮28", "SyL7")])
    )
      ? _0x5ce014[_0x5722("‮29", "exNn")]
      : _0x58ef1d,
    _0x1f2fbe = "";
  if (
    (_0x484f6f = _0x484f6f[_0x5722("‫2a", "tYT]")]) &&
    _0x5ce014["rWvqc"](_0x5ce014[_0x5722("‫2b", "tYT]")], typeof _0x484f6f)
  )
    _0x5f0a7a = _0x484f6f;
  else
    switch (_0x58ef1d) {
  ...
  for (; _0x2b28ff--; )
    _0x1f2fbe +=
      _0x5f0a7a[
        _0x5ce014[_0x5722("‫2f", "w3T]")](
          _0x5ce014[_0x5722("‫30", "xBk^")](
            Math[_0x5722("‮31", "VepR")](),
            _0x5f0a7a[_0x5722("‫32", "fP)@")]
          ),
          0x0
        )
      ];
...
function h5stSign(_0x16a27f) {
  ...
  let _0x137258 = [
    ...
    },
    {
      ...
      value: $[_0x5722("‫3c", "wrSy")]
        ["SHA256"]($[_0x5722("‮3d", "lIgg")](_0x16a27f, _0x16a27f))
        [_0x5722("‮3e", "TZxy")](),
    },
    {
      key: _0x4b101c["dziYL"],
  ...
  let _0x2d7791 = _0x137258["map"](function (_0x2d64bd) {
    return (
      _0x4b101c[_0x5722("‮43", "hoOY")](
        _0x2d64bd[_0x5722("‮44", "rAKg")],
        ":"
      ) + _0x2d64bd[_0x4b101c[_0x5722("‮45", "9I9J")]]
    );
  })[_0x4b101c[_0x5722("‮46", ")UFK")]]("&");
  let _0x4fa26b = Date["now"]();
  let _0x3ed202 = "";
  let _0x2cecaf = format(_0x4b101c["KePDb"], _0x4fa26b);
  _0x3ed202 = $[_0x5722("‫47", "#FOB")](
    $[_0x5722("‮48", "lIgg")],
    $["fp"]["toString"](),
    _0x2cecaf["toString"](),
    _0x4b101c["uXyRi"][_0x5722("‮49", "b17P")](),
    $["CryptoJS"]
  )[_0x5722("‮4a", "7Rz$")]();
  const _0x1c4805 = $["CryptoJS"]
    [_0x5722("‫4b", "%PEG")](_0x2d7791, _0x3ed202[_0x5722("‫4c", "sywN")]())
    [_0x5722("‫4d", "71I(")]();
  let _0x55c9fe = [
    ""["concat"](_0x2cecaf["toString"]()),
    ""["concat"]($["fp"][_0x5722("‫4e", "n$S*")]()),
    ""[_0x5722("‫4f", "zzT%")](
      _0x5722("‫50", "TZxy")[_0x5722("‮4a", "7Rz$")]()
    ),
    ""[_0x5722("‫51", "5nJB")]($[_0x5722("‫52", "w3T]")]),
    ""[_0x5722("‫53", "RRac")](_0x1c4805),
    _0x4b101c["oVeqz"],
    ""[_0x5722("‫54", "TMW@")](_0x4fa26b),
  ][_0x5722("‫55", "7Rz$")](";");
...
function format(_0x3b3946, _0x57ab39) {
    ...
    if (_0x2b4c38["TZrjQ"] !== _0x2b4c38[_0x5722("‮61", "lIgg")]) {
      ...
      let _0x4c69ef = [
        ...
        },
        {
          key: _0x5722("‮65", "rAKg"),
          value: $["CryptoJS"]
            [_0x5722("‫66", "I85n")]($["toStr"](body, body))
            [_0x5722("‮67", "SyL7")](),
        },
        {
      ...
      let _0xe6d7be = _0x4c69ef[_0x5722("‫6b", "9I9J")](function (_0x1a232b) {
        return _0x5c0b26[_0x5722("‮6c", "O*W[")](
          _0x5c0b26[_0x5722("‮6d", "g9zi")](
            _0x1a232b[_0x5c0b26[_0x5722("‮6e", "IzVh")]],
            ":"
          ),
          _0x1a232b[_0x5c0b26["hrRqG"]]
        );
      })[_0x2b4c38[_0x5722("‫6f", "TMW@")]]("&");
      let _0x8d4b31 = Date["now"]();
      let _0x30adc7 = "";
      let _0x4cc82c = _0x2b4c38[_0x5722("‮70", "exNn")](
        format,
        _0x2b4c38[_0x5722("‮71", "jFAu")],
        _0x8d4b31
      );
      _0x30adc7 = $[_0x5722("‫72", "zwqr")](
        $[_0x5722("‮73", "Y6zP")],
        $["fp"]["toString"](),
        _0x4cc82c[_0x5722("‫74", "O*W[")](),
        _0x2b4c38["CUxDE"][_0x5722("‫75", "jFAu")](),
        $[_0x5722("‮76", "SyL7")]
      )[_0x5722("‮77", "WcWE")]();
      const _0x3bef71 = $[_0x5722("‫78", "lIgg")]
        ["HmacSHA256"](_0xe6d7be, _0x30adc7[_0x5722("‮79", "9I9J")]())
        [_0x5722("‮7a", "G^Kg")]();
      let _0x30ebad = [
        ""["concat"](_0x4cc82c[_0x5722("‫7b", "VepR")]()),
        ""[_0x5722("‫7c", "9I9J")]($["fp"][_0x5722("‮7d", "TMW@")]()),
        ""["concat"](
          _0x2b4c38[_0x5722("‮7e", "I85n")][_0x5722("‮7f", "!ydp")]()
        ),
        ""["concat"]($["token"]),
        ""["concat"](_0x3bef71),
        _0x2b4c38["HypbS"],
        ""[_0x5722("‮80", "O*W[")](_0x8d4b31),
      ]["join"](";");
  ...
  var _0xd7a8f2,
    _0x514fe8 = new Date(_0x119460),
    _0x117011 = _0x3b3946,
    _0x589edc = {
      "M+": _0x2b4c38[_0x5722("‫82", "g9zi")](
        _0x514fe8[_0x5722("‫83", "3L1^")](),
        0x1
      ),
      "d+": _0x514fe8["getDate"](),
      ...
      "q+": Math[_0x5722("‫8a", "hoOY")](
        _0x2b4c38[_0x5722("‮8b", "zzT%")](_0x514fe8["getMonth"]() + 0x3, 0x3)
      ),
      "S+": _0x514fe8[_0x5722("‮8c", "G^Kg")](),
    };
  /(y+)/i["test"](_0x117011) &&
    (_0x117011 = _0x117011[_0x5722("‮8d", "3L1^")](
      RegExp["$1"],
      ""
        [_0x5722("‮8e", "qXjd")](_0x514fe8[_0x5722("‫8f", "zzT%")]())
        ["substr"](0x4 - RegExp["$1"][_0x5722("‫90", "9I9J")])
    ));
  Object["keys"](_0x589edc)[_0x5722("‮91", "fP)@")]((_0xd7a8f2) => {
    if (
      new RegExp("("[_0x5722("‮92", "!ydp")](_0xd7a8f2, ")"))[
        _0x5722("‫93", "!ydp")
      ](_0x117011)
    ) {
      var _0x119460,
        _0x3b3946 =
          "S+" === _0xd7a8f2 ? _0x2b4c38[_0x5722("‮94", "cBwY")] : "00";
      _0x117011 = _0x117011["replace"](
        RegExp["$1"],
        0x1 == RegExp["$1"]["length"]
          ? _0x589edc[_0xd7a8f2]
          : ""
              ["concat"](_0x3b3946)
              ["concat"](_0x589edc[_0xd7a8f2])
              [_0x5722("‫95", "rAKg")](
                ""[_0x5722("‮96", "Y6zP")](_0x589edc[_0xd7a8f2])["length"]
              )
      );
    }
  });

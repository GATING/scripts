...
const jdCookieNode = $.isNode() ? require("../jdCookie.jss") : "";
const notify = $.isNode() ? require("../sendNotifyy") : "";
let cookiesArr = [],
  cookie = "",
  message = "";
let ownCode = null;
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = ["a46e8d0d6e57422c9be4564db9f68b2b"];
        ...
        headers: {
          "user-agent": $.isNode()
            ? process.env.JD_USER_AGENT
              ? process.env.JD_USER_AGENT
              : require("../USER_AGENTSS").USER_AGENT
            : $.getdata("JDUA")
            ? $.getdata("JDUA")

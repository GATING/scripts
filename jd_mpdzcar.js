...
async function main() {
  ...
  await getToken();
  if ($.token) {
    await taskPost("getFansInfo", {
      data: $.token,
      source: "01",
      actId: actId,
    });
    if ($.buyerNick) {
      await taskPost("loadUnitedCardActivityInfo", {
        actId: actId,
        buyerNick: $.buyerNick,
      });
      await taskPost("loadSignInState", {
        actId: actId,
        buyerNick: $.buyerNick,
      });
      await $.wait(5000);
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if ($.shopId) {
          await $.wait(5000);
          $.log("关注品牌店铺.. " + $.shopId);
          await taskPost("favouriteShopSingle", {
            actId: actId,
            buyerNick: $.buyerNick,
            shopId: $.shopId,
          });
      ...
      $.log("浏览.. ");
      await taskPost("participantBehavior", {
        actId: actId,
        buyerNick: $.buyerNick,
        behavior: "browseShop",
      });

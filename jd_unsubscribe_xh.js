...
let args_xh = {
   ...
   */
    ...
    (process.env.XH_UNSUB_EXCEPT && process.env.XH_UNSUB_EXCEPT.split("@")) ||
    [],
  /*
   * 是否执行取消关注，默认true
   * 可通过环境变量控制：JD_UNSUB
   ...
   * */
  goodsKeyWords:
    (process.env.JD_UNSUB_GKEYWORDS &&
      process.env.JD_UNSUB_GKEYWORDS.split("@")) ||
    [],
  /*
   ...
   * */
  shopKeyWords:
    (process.env.JD_UNSUB_SKEYWORDS &&
      process.env.JD_UNSUB_SKEYWORDS.split("@")) ||
    [],
  /*
...
!(async () => {
  console.log(
    "X1a0He留：运行前请看好脚本内的注释，日志已经很清楚了，有问题带着日志来问"
  );
  if (args_xh.isRun) {
    if (!cookiesArr[0]) {
      $.msg(
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
      if (cookiesArr[i]) {
        ...
        if (
          !$.endGoods &&
          parseInt($.goodsTotalNum) !== parseInt($.goodsKeyWordsNum)
        )
          await favCommBatchDel();
        //取关商品
        ...
        if (
          !$.endShops &&
          parseInt($.shopsTotalNum) !== parseInt($.shopsKeyWordsNum)
        )
          await batchunfollow();
        //取关店铺
        ...
        do {
          ...
          if (
            parseInt($.goodsTotalNum) === 0 &&
            parseInt($.shopsTotalNum) === 0
          )
            break;
          else {
            //如果商品总数或店铺总数有一个不为0的话，先判断是哪个不为0
            if (parseInt($.goodsTotalNum) !== 0) {
              if (parseInt($.goodsTotalNum) === parseInt($.goodsKeyWordsNum))
                break;
              else {
                $.commIdList = ``;
                await favCommQueryFilter(); //获取商品并过滤
              ...
              if (parseInt($.shopsTotalNum) === parseInt($.shopsKeyWordsNum))
                break;
              else {
                $.shopIdList = ``;
                await queryShopFavList(); //获取店铺并过滤
...
function requireConfig() {
  return new Promise((resolve) => {
    if ($.isNode() && process.env.JD_UNSUB) {
      ...
      console.log(
        `goodPageSize: ${typeof args_xh.goodPageSize}, ${args_xh.goodPageSize}`
      );
      console.log(
        `shopPageSize: ${typeof args_xh.shopPageSize}, ${args_xh.shopPageSize}`
      );
      console.log(
        `goodsKeyWords: ${typeof args_xh.goodsKeyWords}, ${
          args_xh.goodsKeyWords
        }`
      );
      console.log(
        `shopKeyWords: ${typeof args_xh.shopKeyWords}, ${args_xh.shopKeyWords}`
      );
      console.log(
        `unSubscribeInterval: ${typeof args_xh.unSubscribeInterval}, ${
          args_xh.unSubscribeInterval
        }`
      );
      console.log(`printLog: ${typeof args_xh.printLog}, ${args_xh.printLog}`);
      console.log(
        `failTimes: ${typeof args_xh.failTimes}, ${args_xh.failTimes}`
      );
      console.log("=======================");
    }
    resolve();
    ...
    $.get(option, async (err, resp, data) => {
      try {
        if (data.indexOf("Authorization") !== -1) {
          console.log(
            "获取数据失败，401 Authorization Required，可能是User-Agent的问题"
          );
          return;
        }
        ...
        if (data.iRet === "0") {
          ...
          for (let item of data.data) {
            if (
              args_xh.goodsKeyWords.some((keyword) =>
                item.commTitle.includes(keyword)
              )
            ) {
      ...
      try {
        ...
        if (data.iRet === "0") {
          ...
          if (data.data.length > 0) {
            ...
            for (let item of data.data) {
              if (
                args_xh.shopKeyWords.some((keyword) =>

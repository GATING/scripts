      ...
      for (const task of $.taskList) {
        ...
        if (task.taskType === "BROWSE_CHANNEL") {
          ...
          await apDoTask(
            task.id,
            task.taskType,
            encodeURIComponent(task.taskSourceUrl)
          );
          await $.wait(3500);
...
function getTaskList() {
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl("apTaskList", { linkId: linkId }),
      async (err, resp, data) => {
        try {
          if (err) {
...
function apDoTask(taskId, taskType, itemId = "") {
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl("apDoTask", {
        taskType: taskType,
        taskId: taskId,
        channel: 4,
        linkId: linkId,
...
function apTaskDetail(taskId, taskType) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl("apTaskDetail", {
        taskType: taskType,
        taskId: taskId,
        channel: 4,
        linkId: linkId,
...
function apTaskDrawAward(taskId, taskType) {
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl("apTaskDrawAward", {
        taskType: taskType,
        taskId: taskId,
        linkId: linkId,
      }),
...
function starShopPageInfo() {
  ...
  opt = {
    url: `https://api.m.jd.com/?functionId=starShopPageInfo&appid=activities_platform&body=${encodeURIComponent(
      JSON.stringify(body)
    )}&_t=${new Date().getTime()}&cthr=1`,
    headers: {
      "User-Agent": UA,
...
function starShopDraw() {
  body = { linkId: linkId };
  opt = {
    url: `https://api.m.jd.com/?functionId=starShopDraw&appid=activities_platform&body=${encodeURIComponent(
      JSON.stringify(body)
    )}&_t=${new Date().getTime()}&cthr=1`,
    headers: {
      "User-Agent": UA,
...
function taskPostClientActionUrl(
  functionId,
  body,
  ...
  }&appid=activities_platform&body=${JSON.stringify(body)}&t=${time}&cthr=1`;
  if (h5st) {

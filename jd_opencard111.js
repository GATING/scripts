...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = ["3501f1a70d384564b90ebde50fb3492c"];
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      if ($.openCardStatus) {
        ...
        await $.wait(2000);
        await task(
          "dz/openCard/startDraw",
          `activityId=${$.activityId}&actorUuid=${
            $.actorUuid
          }&type=1&pin=${encodeURIComponent($.secretPin)}`
        );
        await $.wait(2000);
        await task(
          "dz/openCard/startDraw",
          `activityId=${$.activityId}&actorUuid=${
            $.actorUuid
          }&type=2&pin=${encodeURIComponent($.secretPin)}`
        );
      } else {
        $.log("没有获取到对应的任务。\n");
      }
...
function getShopOpenCardInfo(body, venderId) {
  let opt = {
    url: `https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=getShopOpenCardInfo&body=${encodeURIComponent(
      JSON.stringify(body)
    )}&client=H5&clientVersion=9.2.0&uuid=88888&h5st=20220316103927741%3B4917820034065113%3B8adfb%3Btk02wac471c5018nzfjQ07QMnzYF7aGYN8INh7pOROYvT9xfhNG4WoAlspfC9wo4vdX6Q79yOggjkDKhiadrPg2z%2B9k%2B%3B034fb6c85703cf98cb40dfa9ecc91fe0c5aefbd69d361721c900544597f535df%3B3.0%3B1647398367741`,
    headers: {
      Host: "api.m.jd.com",
      Accept: "*/*",
      ...
      Referer: `https://shopmember.m.jd.com/shopcard/?venderId=${venderId}}&channel=801&returnUrl=${encodeURIComponent(
        $.activityUrl
      )}`,
...
function bindWithVender(body, venderId) {
  let opt = {
    ...
    headers: {
      ...
      "User-Agent": `jdapp;iPhone;9.5.4;13.6;${$.UUID};network/wifi;ADID/${$.ADID};model/iPhone10,3;addressid/0;appBuild/167668;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 13_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1`,
      "Accept-Language": "zh-cn",
      Referer: `https://shopmember.m.jd.com/shopcard/?venderId=${venderId}}&channel=401&returnUrl=${encodeURIComponent(
        $.activityUrl
      )}`,
      "Accept-Encoding": "gzip, deflate, br",
    },

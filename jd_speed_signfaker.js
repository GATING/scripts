            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0 && data.data) {
                if (data.data.taskInfo.isTaskLimit === 0) {
                  ...
                  console.log(
                    `【${
                      taskCompletionProgress + 1
                    }/${taskCompletionLimit}】浏览商品任务记录成功，等待${videoBrowsing}秒`
                  );
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0 && data.isSuccess) {
                $.score += data.data.reward;
                console.log(
                  `${$.taskName}任务完成，获得${data.data.reward}金币`
                );
              } else {
...
async function cash() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("MyAssetsService.execute", {
...
function wheelsHome() {
  return new Promise((resolve) => {
    $.get(
      taskGetUrl("wheelsHome", { linkId: "toxw9c5sy9xllGBr3QFdYg" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                console.log(
                  `【幸运大转盘】剩余抽奖机会：${data.data.lotteryChances}`
                );
...
function wheelsLottery() {
  return new Promise((resolve) => {
    $.get(
      taskGetUrl("wheelsLottery", { linkId: "toxw9c5sy9xllGBr3QFdYg" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function apTaskList() {
  return new Promise((resolve) => {
    $.get(
      taskGetUrl("apTaskList", { linkId: "toxw9c5sy9xllGBr3QFdYg" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                for (let task of data.data) {
                  ...
                  if (
                    !task.taskFinished &&
                    ["SIGN", "BROWSE_CHANNEL"].includes(task.taskType)
                  ) {
...
function richManIndex() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("richManIndex", { actId: "hbdfw", needGoldToast: "true" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function shootRichManDice() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("shootRichManDice", { actId: "hbdfw" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);

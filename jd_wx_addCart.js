...
if (mode) {
  ...
  $.activityUrl =
    "https://cjhy-isv.isvjcloud.com/wxCollectionActivity/activity?activityId=d138c19abad74f3391ae862d089ab667";
  //一键加购
  $.activityUrl =
    "https://lzkj-isv.isvjcloud.com/wxCollectionActivity/activity2/c3058177366745e08a7884382290b344?activityId=c3058177366745e08a7884382290b344";
}

$.s = 1;
if ($.activityUrl.includes("activityId") > -1) {
  $.activityId =
    ($.activityUrl.match(/activityId=([^&]+)/) &&
      $.activityUrl.match(/activityId=([^&]+)/)[1]) ||
    "";
}
$.domain =
...
$.logic = async function () {
  ...
  let activityContent = await api(
    "wxCollectionActivity/activityContent",
  ...
  if (content.needFollow && !content.hasFollow) {
    await api(
      ...
      `userId=${$.venderId}&activityId=${
        $.activityId
      }&buyerNick=${encodeURIComponent($.Pin)}&activityType=${$.activityType}`
    );
    await $.wait(350, 700);
  }
  ...
  for (let cpvo of $.randomArray(content.cpvos)) {
    ...
    try {
      let carInfo = await api(
        "wxCollectionActivity/addCart",
        `activityId=${$.activityId}&pin=${encodeURIComponent(
          $.Pin
        )}&productId=${cpvo.skuId}`
      );
      if (carInfo.result) {

...
let args_xh = {
  ...
  tabId: (process.env.JD_TRY_TABID &&
    process.env.JD_TRY_TABID.split("@").map(Number)) || [
   ...
   * */
  titleFilters:
    (process.env.JD_TRY_TITLEFILTERS &&
  ...
  if (process.env.JD_TRY && process.env.JD_TRY === "true") {
    ...
    for (let i = 0; i < $.cookiesArr.length; i++) {
      if ($.cookiesArr[i]) {
        ...
        size = 1;
        while (
          trialActivityIdList.length < args_xh.maxLength &&
          $.isForbidden === false
        ) {
        ...
        if ($.isForbidden === false && $.isLimit === false) {
          ...
          for (
            let i = 0;
            i < trialActivityIdList.length && $.isLimit === false;
            i++
          ) {
            if ($.isLimit) {
              console.log("试用上限");
      ...
      if ($.isNode()) {
        if ($.index % args_xh.sendNum === 0) {
          $.sentNum++;
          console.log(
            `正在进行第 ${$.sentNum} 次发送通知，发送数量：${args_xh.sendNum}`
          );
    ...
    if ($.isNode()) {
      if ($.cookiesArr.length - $.sentNum * args_xh.sendNum < args_xh.sendNum) {
        console.log(
          `正在进行最后一次发送通知，发送数量：${
            $.cookiesArr.length - $.sentNum * args_xh.sendNum
          }`
        );
        await $.notify.sendNotify(`${$.name}`, `${notifyMsg}`);
        notifyMsg = "";
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    if (typeof process.env.JD_TRY_WHITELIST === "undefined")
      args_xh.whiteList = false;
    ...
    if (typeof process.env.JD_TRY_UNIFIED === "undefined")
      args_xh.unified = false;
    else args_xh.unified = process.env.JD_TRY_UNIFIED === "true";
    if (typeof process.env.JD_TRY_PASSZC === "undefined")
      args_xh.passZhongCao = true;
    ...
    console.log(
      `totalPages: ${typeof args_xh.totalPages}, ${args_xh.totalPages}`
    );
    ...
    console.log(
      `titleFilters: ${typeof args_xh.titleFilters}, ${args_xh.titleFilters}`
    );
    console.log(
      `trialPrice: ${typeof args_xh.trialPrice}, ${args_xh.trialPrice}`
    );
    console.log(
      `minSupplyNum: ${typeof args_xh.minSupplyNum}, ${args_xh.minSupplyNum}`
    );
    console.log(
      `applyNumFilter: ${typeof args_xh.applyNumFilter}, ${
        args_xh.applyNumFilter
      }`
    );
    console.log(
      `applyInterval: ${typeof args_xh.applyInterval}, ${args_xh.applyInterval}`
    );
    console.log(`maxLength: ${typeof args_xh.maxLength}, ${args_xh.maxLength}`);
    console.log(
      `passZhongCao: ${typeof args_xh.passZhongCao}, ${args_xh.passZhongCao}`
    );
    ...
    console.log(
      `whiteListKeywords: ${typeof args_xh.whiteListKeywords}, ${
    ...
    $.get(option, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data.success) {
            for (let tabId of data.data.tabList)
              console.log(`${tabId.tabName} - ${tabId.tabId}`);
          } else {
      ...
      try {
        if (err) {
          ...
          if (data.success) {
            ...
            console.log(
              `第 ${size++} 次获取试用商品成功，tabId:${
                args_xh.tabId[$.nowTabIdIndex]
              } 的 第 ${page}/${args_xh.totalPages} 页`
            );
              ...
              if (item.skuTitle && $.isPush) {
                args_xh.printLog
                  ? console.log(
                      `检测 tabId:${
                        args_xh.tabId[$.nowTabIdIndex]
                      } 的 第 ${page}/${args_xh.totalPages} 页 第 ${
                        $.nowItem++ + 1
                      } 个商品\n${item.skuTitle}`
                    )
                  : "";
                if (args_xh.whiteList) {
                  if (
                    args_xh.whiteListKeywords.some((fileter_word) =>
                      item.skuTitle.includes(fileter_word)
                    )
                  ) {
                    args_xh.printLog
                      ? console.log(
                          `商品白名单通过，将加入试用组，trialActivityId为${item.trialActivityId}\n`
                        )
                      : "";
                  ...
                  if (parseFloat(item.jdPrice) <= args_xh.jdPrice) {
                    args_xh.printLog
                      ? console.log(
                          `商品被过滤，${item.jdPrice} < ${args_xh.jdPrice} \n`
                        )
                      : "";
                  } else if (
                    parseFloat(item.supplyNum) < args_xh.minSupplyNum &&
                    item.supplyNum !== null
                  ) {
                    args_xh.printLog
                      ? console.log(
                          `商品被过滤，提供申请的份数小于预设申请的份数 \n`
                        )
                      : "";
                  } else if (
                    parseFloat(item.applyNum) > args_xh.applyNumFilter &&
                    item.applyNum !== null
                  ) {
                    args_xh.printLog
                    ...
                    args_xh.printLog
                      ? console.log(
                          `商品通过，将加入试用组，trialActivityId为${item.trialActivityId}\n`
                        )
                      : "";
                    trialActivityIdList.push(item.trialActivityId);
                    trialActivityTitleList.push(item.skuTitle);
            ...
            if (
              page >= args_xh.totalPages &&
              $.nowTabIdIndex < args_xh.tabId.length
            ) {
              ...
              if (data.success && data.data) {
                for (let item of data.data.list) {
                  ...
                  item.status === 2 && item.text.text.includes("试用资格将保留")
                    ...
                    : "";
                  item.status === 2 &&
                  item.text.text.includes("请收货后尽快提交报告")
                    ? ($.getNum += 1)
                    : "";
                ...
                console.log(
                  `待领取 | 已领取 | 已完成 | 已放弃：${$.successNum} | ${$.getNum} | ${$.completeNum} | ${$.giveupNum}`
                );
              } else {
...
function taskurl_xh(appid, functionId, body = JSON.stringify({})) {
  return {
    url: `${URL}?appid=${appid}&functionId=${functionId}&clientVersion=10.3.4&client=wh5&body=${encodeURIComponent(
      body
    )}`,
    headers: {
      Cookie: $.cookie,

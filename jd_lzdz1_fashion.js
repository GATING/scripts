...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = ["feaef1d74f5f44c5bafb2048161cc6ab"];
      $.bean = 0;
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/fashion/union/activity/${
        $.authorNum
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      await task(
        ...
        `venderId=${$.venderId}&code=99&pin=${encodeURIComponent(
          $.secretPin
      ...
      if ($.activityContent) {
        for (let i in $.openInfo) {
          ...
          if (!$.openInfo[i]["openStatus"]) {
            ...
            await getShopOpenCardInfo(
              { venderId: $.openInfo[i]["venderId"], channel: "401" },
              $.openInfo[i]["venderId"]
            );
            await bindWithVender(
              {
                venderId: $.openInfo[i]["venderId"],
        ...
        console.log("任务");
        await task(
          "fashion/union/saveTask",
          `activityId=${$.activityId}&actorUuid=${encodeURIComponent(
            $.actorUuid
          )}&shareUuid=${encodeURIComponent(
            $.authorCode
          )}&pin=${encodeURIComponent($.secretPin)}&taskType=21&taskValue=21`
        );
        await getFirstLZCK();
        await getToken();
        await task(

        ...
        if ($.plantinfo) {
          ...
          if ($.infoArr != "") {
            for (const vo of $.infoArr) {
              ...
              await task(
                "merchant_secondary_pages",
                `shop_id=${vo.shopid}&channel=index`
              );
  ...
  if ($.fertilizerlist && $.fertilizertasklist) {
    if ($.fertilizertasklist.shop) {
      ...
      if ($.fertilizerlist.view_shop === 0) {
        await task(
          "fertilizer_shop_view",
          `shop_id=${$.fertilizertasklist.shop.id}`
        );
        await $.wait(2000);
      } else {
        $.log("任务完成");
      ...
      if (exfertilizer) {
        $.log("美妆币兑换肥料..");
        if (
          $.fertilizerlist.exchange === null ||
          $.fertilizerlist.exchange != "5"
        ) {
          for (let i = 0; i < 5; i++) {
            await task(
              "fertilizer_exchange",
    ...
    if ($.fertilizertasklist.meetingplaces) {
      $.log("去完成逛会员页任务..");
      if (
        $.fertilizertasklist.meetingplaces.length !=
        $.fertilizerlist.view_meetingplace.length
      ) {
        for (const vo of $.fertilizertasklist.meetingplaces) {
          await task(
            "fertilizer_meetingplace_view",
            `meetingplace_id=${vo.id}&shop_id=${vo.shop_id}`
          );
          await $.wait(2000);
        }
      } else {
    ...
    if ($.fertilizertasklist.prodcuts) {
      $.log("去完成加购任务..");
      if (
        $.fertilizertasklist.prodcuts.length !=
        $.fertilizerlist.view_product.length
      ) {
        for (const vo of $.fertilizertasklist.prodcuts) {
          await task(
            "fertilizer_product_view",
            `product_id=${vo.id}&shop_id=${vo.shop_id}`
          );
          await $.wait(2000);
        }
      } else {
...
async function waterdotask() {
  ...
  if ($.tasklist && $.viewlist) {
    ...
    if ($.tasklist.meetingplaces) {
      $.log("去完成浏览会场任务..");
      if (
        $.tasklist.meetingplaces.length != $.viewlist.view_meetingplace.length
      ) {
        ...
        if (data) {
          ...
          switch (function_id) {
            ...
            case "water_shop_view":
              console.log(
                `浏览成功,获得:${data.inc}水滴，总水滴：${data.store_water}滴水`
              );
              break;
            ...
            case "fertilizer_shop_view":
              console.log(
                `浏览成功,获得:${data.inc}肥料，总肥料：${data.store_fertilizer}肥料`
              );
              break;
            ...
            case "fertilizer_exchange":
              if (data.inc) {
                console.log(
                  `兑换成功,获得:${data.inc}肥料，总肥料：${data.store_fertilizer}肥料`
                );
              }
              if (data.status_code === 422) {
                console.log(data.message);
          ...
          if (data) {
            switch (function_id) {
              ...
              case "watering":
                if (data.water) {
                  console.log(
                    `浇水成功,目前总浇水：${data.water}滴,目前等级：${data.level}`
                  );
                }
                if (data.status_code === 422) {
                  console.log(data.message);
              ...
              case "fertilization":
                if (data.status_code === 422) {
                  ...
                  console.log(
                    `施肥成功,目前总施肥：${data.fertilizer}滴,目前等级：${data.level}`
                  );
                }
                break;
              default:

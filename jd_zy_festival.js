    ...
    $.post(MyRequest, async (error, response, data) => {
      try {
        ...
        if (result.data.result.userAwardsCacheDto.type == 2) {
          console.log(
            "\n获得" +
              result.data.result.userAwardsCacheDto.jBeanAwardVo.ext +
              "\n"
          );
          await $.wait(4000);
        } else {
  ...
  return new Promise((resolve) => {
    $.post(MyRequest, async (error, response, data) => {
      try {
        ...
        if (logs) $.log(data);
        if (
          result.data.bizCode == 0 &&
          result.data.result.lotteryReturnCode == 0
        ) {
          console.log(
            "\n获得" +
              result.data.result.userAwardsCacheDto.jBeanAwardVo.prizeName +
              "\n"
          );
          await $.wait(4000);
        } else {
...
async function getlist() {
  ...
  const MyRequest = PostRequest(
    ``,
    `appid=china-joy&functionId=festival_floor_prod&body=%7B%22apiMapping%22%3A%22%2Fapi%2Ftask%2FgetTaskList%22%7D&t=1642671545792&loginType=2`
  );
  return new Promise((resolve) => {
    $.post(MyRequest, async (error, response, data) => {
      try {
...
async function scan() {
  const MyRequest = PostRequest(
    ``,
    `functionId=healthyDay_getHomeData&body=%7B%22appId%22:%221FV1VwKc%22,%22taskToken%22:%22%22,%22channelId%22:1%7D&client=wh5&clientVersion=1.0.0`
  );
  return new Promise((resolve) => {
    $.post(MyRequest, async (error, response, data) => {
      try {
        ...
        if (logs) $.log(data);
        if (result.code == 0) {
          let list6 = result.data.result.taskVos.find(
            (item) => item.taskId == 6
          );
...
async function gethelpcode() {
  const MyRequest = PostRequest(
    ``,
    `functionId=healthyDay_getHomeData&body=%7B%22appId%22%3A%221FV1VwKc%22%2C%22taskToken%22%3A%22%22%2C%22channelId%22%3A1%7D&client=wh5&clientVersion=1.0.0`
  );
  return new Promise((resolve) => {
    $.post(MyRequest, async (error, response, data) => {
      try {
        ...
        if (logs) $.log(data);
        if (result.code == 0) {
          let list1 = result.data.result.taskVos.find(
            (item) => item.taskId == 1
          );
...
async function userScore() {
  const MyRequest = PostRequest(
    ``,
    `functionId=healthyDay_getHomeData&body={"appId":"1FV1VwKc","taskToken":"","channelId":1}&client=wh5&clientVersion=1.0.0`
  );
  return new Promise((resolve) => {
    $.post(MyRequest, async (error, response, data) => {
      try {

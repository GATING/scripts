...
!(async () => {
  ...
  for (let i = 0; i < $.helpCodeList.length && cookiesArr.length > 0; i++) {
    ...
    for (
      let j = 0;
      j < cookiesArr.length && $.helpCodeList[i].needHelp !== 0;
      j++
    ) {
      $.helpFlag = "";
      cookie = cookiesArr[j];
...
async function doOneMission(missionInfo) {
  const body = `[{"userNo":"$cooMrdGatewayUid$","activityCode":"${activityCode}","missionNo":"${
    missionInfo.missionNo
  }","params":${JSON.stringify(missionInfo.params)}}]`;
...
function getPostRequest(type, body) {
  ...
  const headers = {
    ...
    "User-Agent":
      "Mozilla/5.0 (iPhone; CPU iPhone OS 12_0_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/16A404 MicroMessenger/8.0.4(0x1800042c) NetType/4G Language/zh_CN",

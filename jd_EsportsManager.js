    ...
    switch (task_vos.task_name) {
      case "连签得金币":
        if (task_vos.status === "1")
          await do_task(
            task_vos.simple_record_info_vo.task_token,
            task_vos.task_id,
            task_vos.task_type
          );
        continue;
    ...
    $.post(
      {
        ...
        body: `token=${shareCodes[Math.floor(($.index - 1) / 6)].tid}&inviter=${
          Math.floor(($.index - 1) / 6).uid
        }`,
      },
      (err, resp, data) => {
        try {
...
function getIsvToken() {
  let config = {
    ...
    body: `body=${escape(
      '{"action":"to","to":"https%3A%2F%2Fh5.m.jd.com%2FbabelDiy%2FZeus%2F3KSjXqQabiTuD1cJ28QskrpWoBKT%2Findex.html%3FbabelChannel%3D45%26collectionId%3D519"}'
    )}`,
    headers: {
      Host: "api.m.jd.com",
      charset: "UTF-8",
...
function getIsvToken2() {
  let config = {
    ...
    body: `body=${escape(
      '{"id":"","url":"https://xinruidddj-isv.isvjcloud.com"}'
    )}`,
    headers: {
      Host: "api.m.jd.com",
      charset: "UTF-8",
...
function getToken() {
  let config = {
    ...
    headers: {
      ...
      Referer:
        "https://xinruidddj-isv.isvjcloud.com/exception/?channel=DDLY&sid=fd5e44488241862af88cb40cbebf660w&un_area=12_904_3373_62101",
        ...
        try {
          if (!err) {
            ...
            if (data.status === "0") {
              ...
              console.log(
                `任务成功：本次获得 ${result.acquired_score}，账户总额 ${result.user_score}`
              );
              resolve(200);
            } else {
              console.log("任务失败！");

...
活动入口：东东农场->东东乐园(点大风车
好像没啥用 就不知道几滴💧
更新地址：https://raw.githubusercontent.com/GATING/scripts/main/jd_ddnc_farmpark.js
已支持IOS双京东账号, Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, 小火箭，JSBox, Node.js
============Quantumultx===============
[task_local]
#东东乐园
30 7 * * * https://raw.githubusercontent.com/GATING/scripts/main/jd_ddnc_farmpark.js, tag=东东乐园, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "30 7 * * *" script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_ddnc_farmpark.js tag=东东乐园

===============Surge=================
东东乐园 = type=cron,cronexp="30 7 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_ddnc_farmpark.js

============小火箭=========
东东乐园 = type=cron,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_ddnc_farmpark.js, cronexpr="30 7 * * *", timeout=3600, enable=true
...
function browseAward(id, index, type) {
  return new Promise(async (resolve) => {
    const options = taskUrl(
      "ddnc_farmpark_browseAward",
      `{"version":"1","channel":1,"advertId":"${id}","index":${index},"type":${type}}`
    );
    //  console.log(options)
...
function browse(id) {
  return new Promise(async (resolve) => {
    const options = taskUrl(
      "ddnc_farmpark_markBrowser",
      `{"version":"1","channel":1,"advertId":"${id}"}`
    );
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
...
function taskUrl(functionId, body) {
  ...
  return {
    ...
    headers: {
      ...
      Host: "api.m.jd.com",
      Referer:
        "https://h5.m.jd.com/babelDiy/Zeus/J1C5d6E7VHb2vrb5sJijMPuj29K/index.html?babelChannel=ttt1&lng=107.147086&lat=33.255079&sid=cad74d1c843bd47422ae20cadf6fe5aw&un_area=8_573_6627_52446",
      "User-Agent":

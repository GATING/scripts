...
const retainWater = $.isNode()
  ? process.env.retainWater
    ? process.env.retainWater
    : 100
  : $.getdata("retainWater")
  ? $.getdata("retainWater")
  : 100; //保留水滴大于多少g,默认100g;
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      if ($.isNode() && WP_APP_TOKEN_ONE) {
        let tmpMessage = `【京东账号${$.index}】 ${
          $.nickName || $.UserName
        }\n【提醒⏰】初始化农场数据异常, 请登录京东 app查看农场功能是否正常`;
        await notify.sendNotifybyWxPucher(
          `${$.name}`,
          tmpMessage,
          `${$.UserName}`
...
async function getExtraAward() {
  ...
  if ($.farmAssistResult.code === "0") {
    if (
      $.farmAssistResult.assistFriendList &&
      $.farmAssistResult.assistFriendList.length >= 2
    ) {
        ...
        for (let key of Object.keys($.farmAssistResult.assistStageList)) {
          ...
          if (vo.stageStaus === 2) {
            ...
            if ($.receiveStageEnergy.code === "0") {
              console.log(
                `已成功领取第${key + 1}阶段好友助力奖励：【${
                  $.receiveStageEnergy.amount
                }】g水`
              );
    ...
    if (
      $.farmAssistResult.assistFriendList &&
      $.farmAssistResult.assistFriendList.length > 0
    ) {
      let str = "";
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    if ($.isNode()) {
      ...
      console.log(
        `友情提示：设置环境变量 DDNC_FORBID_ACCOUNT="pin1,pin2,pin3,..." 控制哪个京东账号不运行此脚本。多个帐号使用,连接。`
      );
      // 根据pin值过滤帐号
      if (process.env.DDNC_FORBID_ACCOUNT)
        process.env.DDNC_FORBID_ACCOUNT.split(",").forEach((item) => {

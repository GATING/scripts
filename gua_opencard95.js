...
guaopencard_addSku = $.isNode()
  ? process.env.guaopencard_addSku95
    ? process.env.guaopencard_addSku95
    : `${guaopencard_addSku}`
  : $.getdata("guaopencard_addSku95")
  ? $.getdata("guaopencard_addSku95")
  : `${guaopencard_addSku}`;
guaopencard_addSku = $.isNode()
  ...
  : `${guaopencard_addSku}`;
guaopencard = $.isNode()
  ? process.env.guaopencard95
    ? process.env.guaopencard95
    : `${guaopencard}`
  : $.getdata("guaopencard95")
  ? $.getdata("guaopencard95")
  : `${guaopencard}`;
guaopencard = $.isNode()
  ...
  : `${guaopencard}`;
guaopenwait = $.isNode()
  ? process.env.guaopenwait95
    ? process.env.guaopenwait95
    : `${guaopenwait}`
  : $.getdata("guaopenwait95")
  ? $.getdata("guaopenwait95")
  : `${guaopenwait}`;
guaopenwait = $.isNode()
...
guaopencard_draw = $.isNode()
  ? process.env.guaopencard_draw95
    ? process.env.guaopencard_draw95
    : guaopencard_draw
  : $.getdata("guaopencard_draw95")
  ? $.getdata("guaopencard_draw95")
  : guaopencard_draw;
guaopencard_draw = $.isNode()
...
async function run() {
  try {
    ...
    if (
      ...
      nowTime > new Date("2022/02/01 00:00:00+08:00").getTime()
    ) {
      $.activityEnd = true;
      console.log("活动结束");
      return;
    ...
    if (guaopencard_draw + "" !== "0") {
      ...
      console.log('如需抽奖请设置环境变量[guaopencard_draw95]为"3" 3为次数');
          ...
          if (res.result && res.result === true) {
            ...
            for (let i in res.data) {
              ...
              if (item.drawId == "assist") {
                ...
                console.log(
                  `${
                    (item.infoType != 10 && item.drawId && item.drawId + ":") ||
                    ""
                  }${item.infoName}`
                );
              }

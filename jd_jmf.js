...
async function getInteractionInfo(type = true) {
  return new Promise(async (resolve) => {
    $.post(
      taskPostUrl("getInteractionInfo", {
            ...
            if (safeGet(data)) {
              ...
              if (type) {
                ...
                for (let key of Object.keys(
                  data.result.taskPoolInfo.taskList
                )) {
                ...
                if (data.result.hasFinalLottery === 0) {
                  let num = 0;
                  for (let key of Object.keys(
                    data.result.taskPoolInfo.taskRecord
                  )) {
...
function queryPanamaFloor() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("qryCompositeMaterials", {
        geo: { lng: "106.47647010204035", lat: "29.502312842810458" },
        mcChannel: 0,
        activityId: "01235772",
        pageId: "3620025",
        qryParam:
          '[{"type":"advertGroup","id":"06327486","mapTo":"advData","next":[{"type":"productGroup","mapKey":"comment[0]","mapTo":"productGroup","attributes":13}]}]',
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("qryCompositeMaterials", {
        ...
        pageId: "3513123",
        qryParam:
          '[{"type":"advertGroup","id":"06290597","mapTo":"advData","next":[{"type":"productGroup","mapKey":"comment[0]","mapTo":"productGroup","attributes":13}]}]',
        applyKey: "21new_products_h",
      }),
      (err, resp, data) => {
        try {
...
function executeNewInteractionTask(taskType, advertId) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("executeNewInteractionTask", body),
      (err, resp, data) => {
        ...
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} executeNewInteractionTask API请求失败，请检查网路重试`
            );
          } else {
            if (safeGet(data)) {
              data = JSON.parse(data);
              if (data.result.hasDown === 1) {
                console.log(
                  data.result.isLottery === 1
                    ? `找到了一个魔方，获得${
                        data.result.lotteryInfoList[0].quantity || ""
                      }${data.result.lotteryInfoList[0].name}`
                    : `找到了一个魔方`
                );
                $.complete = true;
              }
...
function getNewFinalLotteryInfo() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("getNewFinalLotteryInfo", {
        geo: null,
        mcChannel: 0,
        sign: 3,
        interactionId: $.interactionId,
      ...
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} getNewFinalLotteryInfo API请求失败，请检查网路重试`
            );
          } else {
            if (safeGet(data)) {
              data = JSON.parse(data);
              if (data.result.lotteryStatus === 1) {
                console.log(
                  `开启礼盒成功：获得${data.result.lotteryInfoList[0].quantity}${data.result.lotteryInfoList[0].name}`
                );
              } else {
...
function taskPostUrl(functionId, body = {}) {
  ...
  return {
    url: `${JD_API_HOST}?functionId=${functionId}&body=${encodeURI(
      body
    )}&client=wh5&clientVersion=10.1.4&appid=content_ecology&uuid=${uuid}&t=${Date.now()}`,
    headers: {
      Host: "api.m.jd.com",
      ...
      Referer:
        "https://prodev.m.jd.com/mall/active/TqTRGRrp9HZTfeyRTL2UGmX4mHG/index.html?babelChannel=ttt30",

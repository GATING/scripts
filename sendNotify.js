...
async function sendNotify(
  ...
  try {
    ...
    const notifyGroup2List = process.env.NOTIFY_GROUP2_LIST
      ? process.env.NOTIFY_GROUP2_LIST.split("&")
      : [];
    const titleIndex2 = notifyGroup2List.findIndex((item) => item === strTitle);
    const notifyGroup3List = process.env.NOTIFY_GROUP3_LIST
      ? process.env.NOTIFY_GROUP3_LIST.split("&")
      : [];
    const titleIndexGp3 = notifyGroup3List.findIndex(
      (item) => item === strTitle
    );
    const notifyGroup4List = process.env.NOTIFY_GROUP4_LIST
      ? process.env.NOTIFY_GROUP4_LIST.split("&")
      : [];
    const titleIndexGp4 = notifyGroup4List.findIndex(
      (item) => item === strTitle
    );
    const notifyGroup5List = process.env.NOTIFY_GROUP5_LIST
      ? process.env.NOTIFY_GROUP5_LIST.split("&")
      : [];
    const titleIndexGp5 = notifyGroup5List.findIndex(
      (item) => item === strTitle
    );
    const notifyGroup6List = process.env.NOTIFY_GROUP6_LIST
      ? process.env.NOTIFY_GROUP6_LIST.split("&")
      : [];
    const titleIndexGp6 = notifyGroup6List.findIndex(
      (item) => item === strTitle
    );
    const notifyGroup7List = process.env.NOTIFY_GROUP7_LIST
      ? process.env.NOTIFY_GROUP7_LIST.split("&")
      : [];
    const titleIndexGp7 = notifyGroup7List.findIndex(
      (item) => item === strTitle
    );
              ...
              for (let Tempk = 2; Tempk < strCustomTempArr.length; Tempk++) {
                ...
                switch (strTrmp) {
                  ...
                  case "pushplushxtrip":
                    ...
                    console.log(
                      "自定义设定启用pushplus_hxtrip(推送加)进行通知..."
                    );
                    break;
                  case "Bark":
                    Use_BarkNotify = true;
    ...
    switch (UseGroupNotify) {
      ...
      case 2:
        ...
        if (process.env.BARK_PUSH2 && Use_BarkNotify) {
          if (
            process.env.BARK_PUSH2.indexOf("https") > -1 ||
            process.env.BARK_PUSH2.indexOf("http") > -1
          ) {
            //兼容BARK自建用户
      ...
      case 3:
        ...
        if (process.env.BARK_PUSH3 && Use_BarkNotify) {
          if (
            process.env.BARK_PUSH3.indexOf("https") > -1 ||
            process.env.BARK_PUSH3.indexOf("http") > -1
          ) {
            //兼容BARK自建用户
      ...
      case 4:
        ...
        if (process.env.BARK_PUSH4 && Use_BarkNotify) {
          if (
            process.env.BARK_PUSH4.indexOf("https") > -1 ||
            process.env.BARK_PUSH4.indexOf("http") > -1
          ) {
            //兼容BARK自建用户
      ...
      case 5:
        ...
        if (process.env.BARK_PUSH5 && Use_BarkNotify) {
          if (
            process.env.BARK_PUSH5.indexOf("https") > -1 ||
            process.env.BARK_PUSH5.indexOf("http") > -1
          ) {
            //兼容BARK自建用户
      ...
      case 6:
        ...
        if (process.env.BARK_PUSH6 && Use_BarkNotify) {
          if (
            process.env.BARK_PUSH6.indexOf("https") > -1 ||
            process.env.BARK_PUSH6.indexOf("http") > -1
          ) {
            //兼容BARK自建用户
      ...
      case 7:
        ...
        if (process.env.BARK_PUSH7 && Use_BarkNotify) {
          if (
            process.env.BARK_PUSH7.indexOf("https") > -1 ||
            process.env.BARK_PUSH7.indexOf("http") > -1
          ) {
            //兼容BARK自建用户
    ...
    if (TempList.length > 1) {
      ...
      if (TempCK) {
        ...
        if (strPtPin) {
          ...
          console.log(
            `别名反查PtPin失败: 1.用户更改了别名 2.可能是新用户，别名缓存还没有。`
          );
          return "";
        }
      }

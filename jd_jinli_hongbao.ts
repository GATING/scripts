...
let cookie: string,
  ...
  res: any,
  UserName: string;
let shareCodesSelf: string[] = [],
  shareCodes: string[] = [],
  shareCodesHW: string[] = [],
  fullCode: string[] = [];
let min: number[] = [0.02, 0.03, 0.12, 0.3, 0.4, 0.6, 0.7, 0.8, 1, 1.2, 2, 3.6],
  log: string;
...
!(async () => {
  ...
  cookiesArr = await requireConfig(false);
  cookiesArr = cookiesArr.slice(0, 20);
  if (new Date().getHours() !== 6) await join();
    ...
    try {
      ...
      for (let code of shareCodes) {
        ...
        if (!fullCode.includes(code) && code !== me) {
          console.log(
            `账号${index + 1} ${UserName} 去助力 ${code} ${
              shareCodesSelf.includes(code) ? "*内部*" : ""
            }`
          );
          for (let i = 0; i < 5; i++) {
            if (success) break;
            ...
            if (res.rtn_code === 403) {
              ...
              if (res.data.result.status === 0) {
                console.log(
                  "助力成功：",
                  parseFloat(res.data.result.assistReward.discount)
                );
                await wait(45000);
                remain = false;
                break;
                ...
                if (
                  res.data.result.statusDesc ===
                  "啊偶，TA的助力已满，开启自己的红包活动吧~"
                ) {
                  fullCode.push(code);
                }
                await wait(45000);
...
async function api(fn: string, body: object, retry: number = 0) {
  let { data } = await axios.post(
    `https://api.m.jd.com/api?appid=jinlihongbao&functionId=${fn}&loginType=2&client=jinlihongbao&clientVersion=10.2.4&osVersion=AndroidOS&d_brand=Xiaomi&d_model=Xiaomi`,

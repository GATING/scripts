...
!(async () => {
  ...
  if (!wxgameActivityId) {
    ...
    let data = await getData(
      "https://gitee.com/KingRan521/JD-Scripts/raw/master/shareCodes/jd_game.json"
    );
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.activityShopId = "";
      $.activityUrl = `https://lzkj-isv.isvjcloud.com/wxgame/activity/${
        $.authorNum
    ...
    if ($.secretPin) {
      ...
      await task(
        ...
        `activityId=${$.activityId}&pin=${encodeURIComponent(
          $.secretPin
        )}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent(
          $.pin
      ...
      if ($.activityContent) {
        ...
        if (isdoTask) {
          ...
          await task(
            ...
            `activityId=${$.activityId}&pin=${encodeURIComponent(
              $.secretPin
            )}&taskId=scanshop&param=`
          );
          await task(
            "getProduct",
            `type=3&activityId=${$.activityId}&pin=${encodeURIComponent(
              $.secretPin
            )}`
          );
          for (let j = 0; j < $.getProduct.length; j++) {
            await task(
              ...
              `activityId=${$.activityId}&pin=${encodeURIComponent(
                $.secretPin
              )}&taskId=followsku&param=${$.getProduct[j]["skuId"]}`
            );
          }
          await task(
            "getProduct",
            `type=1&activityId=${$.activityId}&pin=${encodeURIComponent(
              $.secretPin
            )}`
          );
          for (let j = 0; j < $.getProduct.length; j++) {
            await task(
              ...
              `activityId=${$.activityId}&pin=${encodeURIComponent(
                $.secretPin
              )}&taskId=add2cart&param=${$.getProduct[j]["skuId"]}`
            );
          }
        }
            ...
            if ($.gameId) {
              ...
              let gamesign = $.md5(
                $.gameId +
                  ...
                  "," +
                  $.gameScore +
                  ",0eed6538f6e84b754ad2ab95b45c54f8"
              );
              await task(
                "game/end",
                `activityId=${$.activityId}&pin=${encodeURIComponent(
                  $.secretPin
                )}&score=${$.gameScore}&gameId=${
                  $.gameId
                }&reqtime=${reqtime}&sign=${gamesign}&getRank=true&getScoreRank=true&getPlayerNum=true`
              );
            }
            await $.wait(2000);
          } while (!$.stopGame);
...
function taskUrl(function_id, body, isCommon) {
  return {
    url: isCommon
      ...
      : `https://lzkj-isv.isvjcloud.com/wxgame/${function_id}`,
    headers: {

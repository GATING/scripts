...
guaopencard_addSku = $.isNode()
  ? process.env.guaopencard_addSku105
    ? process.env.guaopencard_addSku105
    : `${guaopencard_addSku}`
  : $.getdata("guaopencard_addSku105")
  ? $.getdata("guaopencard_addSku105")
  : `${guaopencard_addSku}`;
guaopencard_addSku = $.isNode()
  ...
  : `${guaopencard_addSku}`;
guaopencard = $.isNode()
  ? process.env.guaopencard105
    ? process.env.guaopencard105
    : `${guaopencard}`
  : $.getdata("guaopencard105")
  ? $.getdata("guaopencard105")
  : `${guaopencard}`;
guaopencard = $.isNode()
  ...
  : `${guaopencard}`;
guaopenwait = $.isNode()
  ? process.env.guaopenwait105
    ? process.env.guaopenwait105
    : `${guaopenwait}`
  : $.getdata("guaopenwait105")
  ? $.getdata("guaopenwait105")
  : `${guaopenwait}`;
guaopenwait = $.isNode()
...
guaopencard_draw = $.isNode()
  ? process.env.guaopencard_draw105
    ? process.env.guaopencard_draw105
    : guaopencard_draw
  : $.getdata("guaopencard_draw105")
  ? $.getdata("guaopencard_draw105")
  : guaopencard_draw;
guaopencard_draw = $.isNode()
...
!(async () => {
  ...
  let shareUuidArr = [
    $.shareUuid,
    "f76b0dcd728548dc8c7bb60dba272f2d",
    "b07290687bec42d7853a56531f6cece3",
    "00828aeec7ce4ebcb28c63a32b1fda34",
    "d9b1c4660a4a400b9225ce9e4c42f08e",
    "1bdc55d58ba74670aa00bc90d19d7c0b",
    "d6bfce0bbd4d41aaa08dbd3f41697da6",
    "50d9a7fcd452463c8668633b01116632",
    "d62d0215e17e4a6890df3e413d5ba403",
  ];
...
async function run() {
  try {
    ...
    if (guaopencard_draw + "" !== "0") {
      ...
      console.log('如需抽奖请设置环境变量[guaopencard_draw105]为"3" 3为次数');
  ...
  switch (type) {
    ...
    case "checkOpenCard":
      ...
      body = `activityId=${$.activityId}&actorUuid=${
        $.actorUuid
      }&pin=${encodeURIComponent($.Pin)}&shareUuid=${$.shareUuid}`;
      break;
    case "getTaskInfos":
    ...
    case "followShop":
      ...
      body = `activityId=${$.activityId}&pin=${encodeURIComponent(
        $.Pin
      )}&actorUuid=${$.actorUuid}&taskType=${$.followShopValue}`;
      break;
    case "addSkus":
      url = `${domain}/tongrentang/union/saveTask`;
      body = `activityId=${$.activityId}&pin=${encodeURIComponent(
      ...
      case "activityContent":
        if (typeof res == "object") {
          if (res.result && res.result === true) {
            ...
            $.actorUuid =
              res.data.actorUuid ||
              (res.data.actorVO && res.data.actorVO.uuid) ||
              "";
      ...
      case "checkOpenCard":
        if (typeof res == "object") {
          if (res.result && res.result === true) {
            ...
            $.allOpenCard =
              res.data.allOpenCard || res.data.allOpenCardStatus || false;
            ...
            if (res.data.sendBeanNum)
              console.log(`开卡获得：${res.data.sendBeanNum}京豆`);
          } else if (res.errorMessage) {
            console.log(`${type} ${res.errorMessage || ""}`);
          } else {
            console.log(`${type} ${data}`);

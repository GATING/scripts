...
!(async () => {
  ...
  for (let i = 0; i < ownCookieNum; i++) {
    if (cookiesArr[i]) {
      ...
      $.activityShopId = "";
      $.activityUrl = `https://lzkjdz-isv.isvjcloud.com/wxShareActivity/activity/${
        $.authorNum
      }?activityId=${$.activityId}&friendUuid=${encodeURIComponent(
        $.authorCode
      )}&shareuserid4minipg=null&shopid=${$.activityShopId}`;
      await share();
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      await task(
        ...
        `venderId=${$.activityShopId}&code=25&pin=${encodeURIComponent(
          $.secretPin
      ...
      await task(
        ...
        `activityId=${$.activityId}&pin=${encodeURIComponent(
          $.secretPin
        )}&friendUuid=${encodeURIComponent($.authorCode)}`
      );
    } else {
      $.log("没有成功获取到用户信息");
    }
    ...
    if ($.secretPin) {
      ...
      for (let d in $.drawContentVOs) {
        await task(
          ...
          `activityId=${$.activityId}&pin=${encodeURIComponent(
            $.secretPin
          )}&drawInfoId=${$.drawContentVOs[d]["drawInfoId"]}`
        );
      }
    } else {
      $.log("没有成功获取到用户信息");
...
function taskUrl(function_id, body, isCommon) {
  return {
    url: isCommon
      ...
      : `https://lzkjdz-isv.isvjcloud.com/wxShareActivity/${function_id}`,
    headers: {

...
exchangeFlag = $.isNode()
  ? process.env.JD_CITY_EXCHANGE
    ? process.env.JD_CITY_EXCHANGE
    : `${exchangeFlag}`
  : $.getdata("JD_CITY_EXCHANGE")
  ? $.getdata("JD_CITY_EXCHANGE")
  : `${exchangeFlag}`;
// 优先助力[助力池]
let helpShareFlag = "true"; //是否优先助力[助力池]，默认是
helpShareFlag = $.isNode()
  ? process.env.JD_CITY_HELPSHARE
    ? process.env.JD_CITY_HELPSHARE
    : `${helpShareFlag}`
  : $.getdata("JD_CITY_HELPSHARE")
  ? $.getdata("JD_CITY_HELPSHARE")
  : `${helpShareFlag}`;
...
!(async () => {
  ...
  if (helpShareFlag + "" == "true") {
    ...
    console.log(
      "脚本优先助力[内部账号] 如需开启优先助力[助力池]，请设置环境变量  JD_CITY_HELPSHARE 为true\n"
    );
  }
  if (exchangeFlag + "" == "true") {
    ...
    console.log(
      `脚本不会自动抽奖，建议活动快结束开启，默认关闭(在1.18日自动开启抽奖),如需自动抽奖请设置环境变量  JD_CITY_EXCHANGE 为true`
    );
  }
  $.inviteIdCodesArr = {};
    ...
    for (let i = 0; i < cookiesArr.length && true; i++) {
      if (cookiesArr[i]) {
        ...
        for (let s = 0; s < cookiesArr.length && true; s++) {
          if (s != $.index - 1 && $.inviteIdCodesArr[s])
            code.push($.inviteIdCodesArr[s]);
        }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0) {
              if (data.data && data["data"]["bizCode"] === 0) {
                console.log(
                  `待提现:￥${data.data.result.userActBaseInfo.poolMoney}`
                );
                for (let vo of (data.data.result &&
                  data.data.result.popWindows) ||
                  []) {
                ...
                if (flag) {
                  ...
                  for (let vo of (data.data.result &&
                    data.data.result.taskInfo.taskDetailResultVo.taskVos &&
                    false) ||
                    []) {
                    if (vo && vo.status == 1) {
...
function readShareCode() {
  ...
  return new Promise(async (resolve) => {
    $.get(
      { url: `https://jd.smiek.tk/city`, timeout: 10000 },
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);

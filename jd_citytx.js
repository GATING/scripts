...
!(async () => {
  ...
  if (redPacket) {
    ...
    console.log(
      `脚本不会自动红包提现，建议抢不到微信提现时 配置  JD_CITY_RED_PACKET 为true 改动红包提现`
    );
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      for (
        let j = parseFloat(jdWithdrawAmount);
        j <= parseFloat(money);
        j = j + parseFloat(jdWithdrawAmount)
      ) {
        await $.wait(1000);
        console.log("开始提现" + j);
        await city_withdraw();
  ...
  return new Promise((resolve) => {
    ...
    let option = {
      ...
      body:
        "functionId=" +
        function_id +
        "&body=&client=wh5&clientVersion=1.0.0&uuid=network/wifi",
    };
    $.post(option, (err, resp, data) => {
      try {
        ...
        if (data.data.success) {
          ...
          console.log(
            "邀请链接：	https://bunearth.m.jd.com/babelDiy/Zeus/x4pWW6pvDwW7DjxMmBbnzoub8J/index.html?inviteId=" +
              res.userActBaseInfo.inviteId
          );

...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/tiger/gold/activity/${
        $.authorNum
        ...
        if (!$.activityContent.followShopStatus) {
          ...
          await task(
            ...
            `activityId=${$.activityId}&pin=${encodeURIComponent(
              ...
              $.actorUuid
            )}&taskType=23&taskValue=&shareUuid=${encodeURIComponent(
              $.authorCode
            )}`
          );
        } else {
...
function getShopOpenCardInfo(venderId) {
  let opt = {
    url: `https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=getShopOpenCardInfo&body=${encodeURIComponent(
      JSON.stringify({ venderId: venderId, channel: 401 })

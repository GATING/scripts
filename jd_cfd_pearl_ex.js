...
const UA =
  ...
  `jdpingou;iPhone;4.13.0;14.4.2;${randomString(
    40
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      await perl_auto();
      console.log(
        `请求兑换API后时间 ${new Date().Format("yyyy-MM-dd hh:mm:ss | S")}`
      );
    }
  }
})()
...
async function perl_auto() {
  try {
    ...
    let prizes = [
      ...$.perl_data.exchangeInfo.prizeInfo,
      $.perl_data.exchangeInfo.randHbPrizeInfo,
    ].filter((prize) => {
      ...
      if (flag) {
        if (prize.dwState == 3) {
          ...
          !prize.strPrizeName ||
          prize.ddwVirHb <= (process.env.ddwVirHb || 0.2) * 100
        ) {
          flag = false;
          console.log(
            strPrizeName,
            "不大于",
            process.env.ddwVirHb || 0.2,
            "元 过滤"
          );
...
async function refresh_perl() {
  return new Promise(async (resolve) => {
    ...
    let url = `${JD_API_HOST}jxbfd/user/ExchangePearlState?__t=${
      timestamp + 2
    }&strZone=jxbfd&dwExchangeType=undefined&_ste=1`;
    url += `&h5st=${decrypt(timestamp, "", "", url)}&_=${
      timestamp + 2
...
function taskUrl_perl(function_path, dwLvl, dwIsRandHb, ddwVirHb, strPoolName) {
  const timestamp = Date.now();
  let url = `${JD_API_HOST}jxbfd/${function_path}?__t=${
    timestamp + 2
  }&strZone=jxbfd&dwLvl=${dwLvl}&dwIsRandHb=${dwIsRandHb}&ddwVirHb=${ddwVirHb}&strPoolName=${strPoolName}&dwExchangeType=${
    $.dwExchangeType
  }&_stk=__t%2CddwVirHb%2CdwExchangeType%2CdwIsRandHb%2CdwLvl%2CstrPoolName%2CstrZone&_ste=1`;
  url += `&h5st=${decrypt(timestamp, "", "", url)}&_=${
    timestamp + 2
  }&sceneval=2&g_login_type=1`;
  // console.debug('taskUrl_perl:',url)

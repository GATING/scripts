  ...
  if ($.info.data.firstJoinFlag === true) {
    ...
    if (dlbtz == true) {
      allMessage += `京东账号${$.index}-${
        ...
        $.info.data.plantInfo[0].nowStep
      }\n当前下一阶段还需要浇水：${
        $.info.data.plantInfo[0].upgradeWateringNum
      }次${$.index !== cookiesArr.length ? "\n\n" : "\n\n"}`;
    }
    if (getwat.code === 0) {
  ...
  return new Promise(async (resolve) => {
    let options = {
      ...
      headers: {
        ...
        "User-Agent":
          "Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.5(0x18000528) NetType/WIFI Language/zh_CN",
        Cookie: cookie,
      },
    };

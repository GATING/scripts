...
let cookiesArr = [],
  cookie = "",
  message = "",
  linkId = "ouLEuSSoBzj9b9YYYIsiDA",
  fflLinkId = "ouLEuSSoBzj9b9YYYIsiDA";
...
function gambleHomePage() {
  ...
  const options = {
    url: `https://api.m.jd.com/?functionId=gambleHomePage&body=${encodeURIComponent(
      JSON.stringify(body)
    )}&appid=activities_platform&clientVersion=null`,
    headers,
  };
...
function gambleOpenReward() {
  ...
  const options = {
    ...
    body: `functionId=gambleOpenReward&body=${JSON.stringify(
      body
    )}&t=${Date.now()}&appid=activities_platform&clientVersion=null`,
  };
  return new Promise((resolve) => {

...
if (mode) {
  $.activityUrl =
    "https://lzkjdz-isv.isvjcloud.com/wxCollectCard/activity/2149304?activityId=84a8b50d3a0c48f9bc7804be9da5deac";
  $.activityUrl =
    "https://lzkjdz-isv.isvjcloud.com/wxCollectCard/activity/1193422?activityId=cb4d9c7ca992427db5a52ec1c0bcc42e";
}
...
$.logic = async function () {
  ...
  $.domain =
    ($.activityUrl.match(/https?:\/\/([^/]+)/) &&
      $.activityUrl.match(/https?:\/\/([^/]+)/)[1]) ||
    "";
  ...
  $.domain.includes("lzkj-isv.isvjcloud.com")
    ? await api("wxCommonInfo/initActInfo", `activityId=${$.activityId}`)
    : "";
  ...
  shopInfo = await api(`wxCollectCard/shopInfo`, `activityId=${$.activityId}`);
  if (!shopInfo.result) {
  ...
  let activityContent = await api(
    ...
    `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}&uuid=${
      $.shareUuid
    }`
  );
  ...
  let drawCount =
    ($.match(/每人每天可获得(\d+)次/, activityContent.data.rule) &&
      $.match(/每人每天可获得(\d+)次/, activityContent.data.rule) * 1) ||
    5;
  console.log("openCard", activityContent.data.openCard);
  $.shareUuid = $.shareUuid || activityContent.data.uuid;

  let drawContent = await api(
    "wxCollectCard/drawContent",
    `activityId=${$.activityId}`
  );
  ...
  await api(
    ...
    `venderId=${$.venderId}&elementId=${encodeURIComponent("邀请")}&pageId=${
      $.activityId
    }&pin=${encodeURIComponent($.Pin)}`
  );

  await api(
    "wxCollectCard/drawCard",
    `sourceId=${$.shareUuid}&activityId=${
      $.activityId
    }&type=1&pinImg=${encodeURIComponent(
      $.attrTouXiang
    )}&pin=${encodeURIComponent($.Pin)}&jdNick=${encodeURIComponent(
      $.nickname
    )}`
  );
  let saveSource = await api(
    "wxCollectCard/saveSource",
  ...
  for (let i = 0; i < drawCount; i++) {
    ...
    if (prize.result) {
      ...
      if (prize.errorMessage.includes("上限")) {
        ...
        prize.errorMessage.includes("来晚了") ||
    ...
    await api(
      "crm/pageVisit/insertCrmPageVisit",
      `venderId=${$.venderId}&elementId=${encodeURIComponent("抽卡")}&pageId=${
        $.activityId
      }&pin=${encodeURIComponent($.Pin)}`
    );

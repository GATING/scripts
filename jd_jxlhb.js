...
!(async () => {
  ...
  let res = await getAuthorShareCode(
    "https://gitee.com/KingRan521/JD-Scripts/blob/master/shareCodes/jxlhb.json"
  );
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let j = 0; j < $.packetIdArr.length && $.canHelp; j++) {
      console.log(
        `【${$.UserName}】去助力【${$.packetIdArr[j].userName}】邀请码：${$.packetIdArr[j].strUserPin}`
      );
    ...
    if ($.canHelp && $.authorMyShareIds && $.authorMyShareIds.length) {
      ...
      for (let j = 0; j < $.authorMyShareIds.length && $.canHelp; j++) {
        console.log(
          `【${$.UserName}】去助力作者的邀请码：${$.authorMyShareIds[j]}`
        );
        $.max = false;
...
function joinActive() {
  return new Promise((resolve) => {
    $.get(
      taskurl(
        "JoinActive",
        `stepreward_jstoken=${token["farm_jstoken"]}&timestamp=${token["timestamp"]}&phoneid=${token["phoneid"]}&strPin=`
      ),
      (err, resp, data) => {
        try {
          if (err) {
...
function enrollFriend(strPin) {
  return new Promise((resolve) => {
    $.get(
      taskurl(
        "EnrollFriend",
        `stepreward_jstoken=${token["farm_jstoken"]}&timestamp=${token["timestamp"]}&phoneid=${token["phoneid"]}&strPin=${strPin}`
      ),
      (err, resp, data) => {
        try {
          if (err) {
...
function taskurl(function_path, body = "") {
  let url = `${BASE_URL}/${function_path}?activeId=${$.activeId}${
    body ? `&${body}` : ""
  }`;
  url += `&_stk=${getStk(url)}`;
  url += `&_ste=1&h5st=${decrypt(
    ...
    "",
    url
  )}&userDraw=0&publishFlag=1&channel=7&_t=${Date.now()}&_=${Date.now()}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(

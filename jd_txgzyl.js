...
async function activity(timeout = 500) {
  return new Promise((resolve) => {
    setTimeout(() => {
      let url = {
        url: `https://lzkj-isv.isvjcloud.com/wxShopGift/activity?activityId=${activityId}&sid=${randomString(
          32,
          "xx"
        )}&un_area=${randomString(2, "int")}_${randomString(
          4,
          "int"
        )}_${randomString(4, "int")}_${randomString(5, "int")}`,
        ...
        headers: {
          ...
          "User-Agent": $.isNode()
            ...
            : $.getdata("JDUA")
            ? $.getdata("JDUA")
            : "Mozilla/5.0 (iPhone; CPU iPhone OS 15_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.18(0x18001236) NetType/WIFI Language/zh_CN",
    ...
    setTimeout(() => {
      let url = {
        ...
        headers: {
          ...
          Host: `lzkj-isv.isvjcloud.com`,
          Referer: `https://lzkj-isv.isvjcloud.com/wxShopGift/activity?activityId=${activityId}&sid=${randomString(
            32,
            "xx"
          )}w&un_area=${randomString(2, "int")}_${randomString(
            4,
            "int"
          )}_${randomString(4, "int")}_${randomString(5, "int")}`,
          "Accept-Language": `zh-CN,zh-Hans;q=0.9`,
          ...
          if ($.data.result) {
            ...
            if (lp) {
              lp_list = lp
                ...
                .replace("dq", "东券");
              console.log(
                `${$.nickName || $.UserName} 哇！快看，特效真美美美，biubiu~`
              );
            }
...
async function isvObfuscator(timeout = 500) {
  return new Promise((resolve) => {
    setTimeout(() => {
      body = `body=%7B%22url%22%3A%22https%3A%5C/%5C/lzkj-isv.isvjcloud.com%22%2C%22id%22%3A%22%22%7D&build=167874&client=apple&clientVersion=10.2.4&d_brand=apple&d_model=iPhone14%2C3&ef=1&eid=${randomString(
        116
      )}&ep=%7B%22ciphertype%22%3A5%2C%22cipher%22%3A%7B%22screen%22%3A%22CJS4DMeyDzc4%22%2C%22wifiBssid%22%3A%22${randomString(
        43
      )}%3D%22%2C%22osVersion%22%3A%22CJUkDK%3D%3D%22%2C%22area%22%3A%22${randomString(
        24
      )}%22%2C%22openudid%22%3A%22DtVwZtvvZJcmZwPtDtc5DJSmCtZvDzLsCzK2DJG2DtU1EWG5Dzc2ZK%3D%3D%22%2C%22uuid%22%3A%22${randomString(
        32,
        "xx"
      )}%22%7D%2C%22ts%22%3A${get_times("ss")}%2C%22hdid%22%3A%22${randomString(
        43
      )}%3D%22%2C%22version%22%3A%221.0.3%22%2C%22appname%22%3A%22com.360buy.jdmobile%22%2C%22ridx%22%3A-1%7D&ext=%7B%22prstate%22%3A%220%22%7D&isBackground=N&joycious=98&lang=zh_CN&networkType=wifi&networklibtype=JDNetworkBaseAF&partner=apple&rfs=0000&scope=10&sign=ece45b391fb4abf4e4590c7da6eeacc5&st=1649150743509&sv=101&uemps=0-0&uts=${randomString(
        64
      )}`;
      let url = {
...
function randomString(len, lx = "hh") {
  ...
  if (lx === "hh") {
    var $chars =
      "ABCDEFGHJKMNPQRSTWXYZoOLlabcde9gqfhijkmnVvprstwxyz234Uu5I1678"; /****默认去掉了容易混淆的字符oOLl,9gq,Vv,Uu,I1****/
  } else if (lx === "xx") {
    var $chars =
      "abcdefhijkmnprstwxyzolgqvu0192345678"; /****默认去掉了容易混淆的字符oOLl,9gq,Vv,Uu,I1****/
  } else if (lx === "dx") {
        ...
        try {
          if (err) {
            ...
            if (data.ok) {
              ...
              $.msg(
                `【PKC提示】`,
                `[${$.name}]变量已推送到监控群组【${data.result.chat.title}】\n`
              );
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    const activityIdArrNode = $.isNode()
      ? process.env.PKC_TXGZYL.split("@")
      : [];
    ...
    if ($.isNode()) {
      ...
      if ($.getdata("pkc_txgzyl"))
        $.activityIdArr = $.getdata("pkc_txgzyl")
          .split("@")
          .filter((item) => !!item);

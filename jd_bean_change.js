...
if ($.isNode()) {
  ...
  if (process.env.BEANCHANGE_ExJxBeans == "true") {
    if (time >= 17) {
      ...
      console.log(
        `检测到设定了临期京豆转换喜豆,但时间未到17点后，暂不执行转换...`
      );
    }
  }
}
...
let DisableIndex = -1;
if ($.isNode()) {
  strDisableList = process.env.BEANCHANGE_DISABLELIST
    ? process.env.BEANCHANGE_DISABLELIST.split("&")
    : [];
}
...
!(async () => {
  ...
  for (i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      //汪汪乐园
      if (EnableJoyPark) await getJoyBaseInfo();

      //京东赚赚
      if (EnableJdZZ) await getJdZZ();

      //京东秒杀
      if (EnableJdMs) await getMs();
      ...
      //京喜工厂
      if (EnableJxGC) await getJxFactory();

      // 京东工厂
      if (EnableJDGC) await getDdFactoryInfo();

      //领现金
      if (EnableCash) await jdCash();
      ...
      //金融养猪
      if (EnablePigPet) await GetPigPetInfo();
...
async function showMsg() {
  ...
  if ($.levelName || $.JingXiang) {
    ...
    if ($.levelName) {
      ...
      if ($.isPlusVip == 1) ReturnMessage += `${$.levelName}Plus`;
      else ReturnMessage += `${$.levelName}会员`;
    }
  ...
  if (EnableJxBeans) {
    ...
    ReturnMessage += `\n`;
    ReturnMessage += `【当前喜豆】${$.xibeanCount}喜豆(≈${(
      $.xibeanCount / 100
    ).toFixed(2)}元)\n`;
    strsummary += `【当前喜豆】${$.xibeanCount}豆(≈${(
      $.xibeanCount / 100
    ).toFixed(2)}元)\n`;
  }
  ...
  if ($.JDtotalcash) {
    ReturnMessage += `【极速金币】${$.JDtotalcash}币(≈${(
      $.JDtotalcash / 10000
    ).toFixed(2)}元)\n`;
  }
  if ($.JdzzNum) {
    ReturnMessage += `【京东赚赚】${$.JdzzNum}币(≈${($.JdzzNum / 10000).toFixed(
      2
    )}元)\n`;
  }
  if ($.JdMsScore != 0) {
    ReturnMessage += `【京东秒杀】${$.JdMsScore}币(≈${(
      $.JdMsScore / 1000
    ).toFixed(2)}元)\n`;
  }
  ...
  if ($.JdFarmProdName != "") {
    if ($.JdtreeEnergy != 0) {
      if ($.treeState === 2 || $.treeState === 3) {
        ...
        if (userIndex2 != -1) {
          ReceiveMessageGp2 += `【账号${IndexGp2} ${
            $.nickName || $.UserName
          }】${$.JdFarmProdName} (东东农场)\n`;
        }
        if (userIndex3 != -1) {
          ReceiveMessageGp3 += `【账号${IndexGp3} ${
            $.nickName || $.UserName
          }】${$.JdFarmProdName} (东东农场)\n`;
        }
        if (userIndex4 != -1) {
          ReceiveMessageGp4 += `【账号${IndexGp4} ${
            $.nickName || $.UserName
          }】${$.JdFarmProdName} (东东农场)\n`;
        }
        if (userIndex2 == -1 && userIndex3 == -1 && userIndex4 == -1) {
          allReceiveMessage += `【账号${IndexAll} ${
            $.nickName || $.UserName
          }】${$.JdFarmProdName} (东东农场)\n`;
        }
      } else {
      ...
      if ($.treeState === 0) {
        ...
        if (userIndex2 != -1) {
          WarnMessageGp2 += `【账号${IndexGp2} ${
            $.nickName || $.UserName
          }】水果领取后未重新种植! (东东农场)\n`;
        }
        if (userIndex3 != -1) {
          WarnMessageGp3 += `【账号${IndexGp3} ${
            $.nickName || $.UserName
          }】水果领取后未重新种植! (东东农场)\n`;
        }
        if (userIndex4 != -1) {
          WarnMessageGp4 += `【账号${IndexGp4} ${
            $.nickName || $.UserName
          }】水果领取后未重新种植! (东东农场)\n`;
        }
        if (userIndex2 == -1 && userIndex3 == -1 && userIndex4 == -1) {
          allWarnMessage += `【账号${IndexAll} ${
            $.nickName || $.UserName
          }】水果领取后未重新种植! (东东农场)\n`;
        }
      } else if ($.treeState === 1) {
  ...
  if ($.DdFactoryReceive) {
    if (userIndex2 != -1) {
      ReceiveMessageGp2 += `【账号${IndexGp2} ${$.nickName || $.UserName}】${
        $.DdFactoryReceive
      } (东东工厂)\n`;
    }
    if (userIndex3 != -1) {
      ReceiveMessageGp3 += `【账号${IndexGp3} ${$.nickName || $.UserName}】${
        $.DdFactoryReceive
      } (东东工厂)\n`;
    }
    if (userIndex4 != -1) {
      ReceiveMessageGp4 += `【账号${IndexGp4} ${$.nickName || $.UserName}】${
        $.DdFactoryReceive
      } (东东工厂)\n`;
    }
    if (userIndex2 == -1 && userIndex3 == -1 && userIndex4 == -1) {
      ...
      if (
        ...
        if ($.petInfo.userStatus === 0) {
          ...
          if (userIndex2 != -1) {
            ReceiveMessageGp2 += `【账号${IndexGp2} ${
              $.nickName || $.UserName
            }】${$.petInfo.goodsInfo.goodsName}可以兑换了! (东东萌宠)\n`;
          }
          if (userIndex3 != -1) {
            ReceiveMessageGp3 += `【账号${IndexGp3} ${
              $.nickName || $.UserName
          ...
          ReturnMessage += `(${response.result.medalPercent.toFixed(0)}%,${
            response.result.medalNum
          }/${
...
async function Monthbean() {
  ...
  console.log(
    `计算月京豆起始日期:` +
      GetDateTime(new Date(year + "-" + month + "-01 00:00:00"))
  );
  ...
  console.log(
    `计算月京豆结束日期:` +
      GetDateTime(new Date(year + "-" + (month + 1) + "-01 00:00:00"))
  );
        ...
        if (err) {
          ...
          if (data) {
            ...
            if ($.jxRed > 0)
              $.message += `【京喜红包】${
                $.jxRed
              }(将过期${$.jxRedExpire.toFixed(2)})元 \n`;
            if ($.jsRed > 0)
              $.message += `【极速红包】${
                $.jsRed
              }(将过期${$.jsRedExpire.toFixed(2)})元 \n`;
            if ($.jdRed > 0)
              $.message += `【京东红包】${
                $.jdRed
              }(将过期${$.jdRedExpire.toFixed(2)})元 \n`;
            if ($.jdhRed > 0)
              $.message += `【健康红包】${
                $.jdhRed
              }(将过期${$.jdhRedExpire.toFixed(2)})元 \n`;
          } else {
            console.log(`京东服务器返回空数据`);
          }
        }
...
function taskcashUrl(_0x7683x2, _0x7683x3 = {}) {
  ...
  return {
    url: `${__Oxb24bc[0x8]}${JD_API_HOST}${__Oxb24bc[0x9]}${_0x7683x2}${
      __Oxb24bc[0xa]
    }${escape(JSON[__Oxb24bc[0x1]](_0x7683x3))}${__Oxb24bc[0xb]}${_0x7683x4}${
      __Oxb24bc[0xc]
    }${_0x7683x8}${__Oxb24bc[0x8]}`,
    headers: {
      Host: __Oxb24bc[0xd],
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  if (production.investedElectric >= production.needElectric) {
                    ...
                    strTemp = `,${(
                      (production.needElectric - production.investedElectric) /
                      (2 * 60 * 60 * 24)
                    ).toFixed(0)}天)`;
                    if (strTemp == ",0天)") infoMsg += ",今天)";
                    else infoMsg += strTemp;
                  }
...
function jxTaskurl(functionId, body = "", stk) {
  let url = `https://m.jingxi.com/dreamfactory/${functionId}?zone=dream_factory&${body}&sceneval=2&g_login_type=1&_time=${Date.now()}&_=${
    Date.now() + 2
  }&_ste=1`;
...
async function getDdFactoryInfo() {
  ...
  return new Promise((resolve) => {
    $.post(
      ddFactoryTaskUrl("jdfactory_getHomeData"),
      async (err, resp, data) => {
        try {
          if (err) {
            $.ddFactoryInfo = "获取失败!";
...
function GetJxBeanDetailData() {
  return new Promise((resolve) => {
    $.get(
      taskJxUrl("queryuserjingdoudetail", "pagesize=10&type=16"),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(JSON.stringify(err));

...
if (mode) {
  $.activityUrl =
    "https://lzkj-isv.isvjcloud.com/wxDrawActivity/activity?activityId=3df02d007f4a474986d048036ef643e2";
}
let stop = false;
$.s = 1;
$.logic = async function () {
  ...
  if (typeof lzToken.data == "string") {
    if (
      (lzToken.data.match(/(活动已经结束)/) &&
        lzToken.data.match(/(活动已经结束)/)[1]) ||
      ""
    ) {
      $.putMsg("活动已结束");
    } else {
  ...
  let shopInfo = await api(
    "wxDrawActivity/shopInfo",
    `activityId=${$.activityId}`,
    true
  );
  if (!shopInfo.result) {
  ...
  if ($.startTime > 0 && $.startTime > $.timestamp()) {
    $.putMsg(
      `抽奖活动开始时间${$.formatDate($.startTime, "yyyy-MM-dd HH:mm:ss")}`
    );
    stop = true;
    return;
  }
  ...
  $.log(
    `抽奖次数 ${$.canDrawTimes} ${
      ($.drawConsume > 0 && $.drawConsume + "积分抽奖一次") || ""
    }`
  );
  if ($.canDrawTimes === 0) {
    return;
  ...
  if ($.needFollow && !$.hasFollow) {
    let fallow = await api(
      $.domain.includes("cjhy-isv.isvjcloud.com")
        ? "/wxActionCommon/newFollowShop"
        : "/wxActionCommon/followShop",
      $.domain.includes("cjhy-isv.isvjcloud.com")
        ? `venderId=${$.userId}&activityId=${
            $.activityId
          }&buyerPin=${encodeURIComponent(
            encodeURIComponent($.Pin)
          )}&activityType=${$.activityType}`
        : `userId=${$.userId}&activityId=${
            $.activityId
          }&buyerNick=${encodeURIComponent($.Pin)}&activityType=${
            $.activityType
          }`
    );
    if (fallow.result) {
  ...
  if (data.success) {
    if (data.result && data.result.giftInfo) {
      for (let i of data.result.giftInfo.giftList) {
        $.log(`入会获得:${i.discountString}${i.prizeName}${i.secondLineDesc}`);
      }
    }

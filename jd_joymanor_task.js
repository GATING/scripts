  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      for (const task of $.taskList) {
        ...
        if (
          task.taskType === "BROWSE_PRODUCT" ||
          (task.taskType === "BROWSE_CHANNEL" && task.taskLimitTimes !== 1)
        ) {
          ...
          while (task.taskLimitTimes - task.taskDoTimes >= 0) {
            if (productList.length === 0) {
              $.log(
                `${task.taskTitle} 活动火爆，素材库没有素材，我也不知道啥回事 = = `
              );
              break;
            }
            $.log(
            ...
            let resp = await apDoTask(
              task.id,
              task.taskType,
              productList[productListNow].itemId,
              productList[productListNow].appid
            );
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"linkId":"Vr5e5qokLNCRxNmi4VTW4Q"}&appid=activities_platform`,
        `apTaskList`
      ),
      async (err, resp, data) => {
        $.log("=== 任务列表 start ===");
        try {
          if (err) {
            ...
            for (const row of $.taskList) {
              $.log(
                `${row.taskTitle} ${row.taskDoTimes}/${row.taskLimitTimes}`
              );
            }
            $.log("=== 任务列表 end  ===");
          }
...
function getJoyBaseInfo(taskId = "", inviteType = "", inviterPin = "") {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"taskId":"${taskId}","inviteType":"${inviteType}","inviterPin":"${inviterPin}","linkId":"Vr5e5qokLNCRxNmi4VTW4Q"}&appid=activities_platform&t=${Date.now()}&client=activities_platform&h5st=20220509105406529%3B9112909836479988%3B4abce%3Btk02w774b1bbd18nGS0GgF00CXWFenyTPHs%2Bp26eYf9ZmlZVf%2BvEe3Gf0Zd80IGUasS%2FWd%2FoZlsJdtXKnoeXyciIpR1U%3B367214d88c1dbeda0cee50036420f60572854e94d718de89adee91dc59e42668%3B3.0%3B1652064846529&cthr=1`,
        `joyBaseInfo`
      ),
      async (err, resp, data) => {
        try {
...
function apDoTask(
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"taskType":"${taskType}","taskId":${taskId},"channel":4,"linkId":"Vr5e5qokLNCRxNmi4VTW4Q","itemId":"${itemId}"}&appid=${appid}`,
        `apDoTask`
      ),
      async (err, resp, data) => {
        try {
...
function apDoTask2(taskId, taskType, itemId, appid = "activities_platform") {
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"taskType":"${taskType}","taskId":${taskId},"linkId":"Vr5e5qokLNCRxNmi4VTW4Q","itemId":"${itemId}"}&appid=${appid}`,
        `apDoTask`
      ),
      async (err, resp, data) => {
        try {
...
function apTaskDetail(taskId, taskType) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `functionId=apTaskDetail&body={"taskType":"${taskType}","taskId":${taskId},"channel":4,"linkId":"Vr5e5qokLNCRxNmi4VTW4Q"}&appid=activities_platform`,
        `apTaskDetail`
      ),
      async (err, resp, data) => {
        try {
...
function apTaskDrawAward(taskId, taskType) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"taskType":"${taskType}","taskId":${taskId},"linkId":"Vr5e5qokLNCRxNmi4VTW4Q"}&appid=activities_platform`,
        `apTaskDrawAward`
      ),
      async (err, resp, data) => {
        try {
...
function taskPostClientActionUrl(body, functionId) {
  return {
    ...
    headers: {
      ...
      Referer:
        "https://joypark.jd.com/?activityId=Vr5e5qokLNCRxNmi4VTW4Q&lng=113.387899&lat=22.512678&sid=4d76080a9da10fbb31f5cd43396ed6cw&un_area=19_1657_52093_0",
      Cookie: cookie,
    },
  };
}

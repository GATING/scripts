...
async function getTaskList() {
  return new Promise((resolve) => {
    $.post(
      api(
        'appid=china-joy&functionId=festival_floor_prod&body={"apiMapping":"/api/task/getTaskList"}&t=1643262434906&loginType=2'
      ),
...
async function doapi(taskId, taskItemId) {
  return new Promise((resolve) => {
    $.post(
      api(
        `appid=china-joy&functionId=festival_floor_prod&body={"taskId":"${taskId}","taskItemId":"${taskItemId}","apiMapping":"/api/task/doTask"}&t=${timestamp}&loginType=2`
      ),
      async (error, response, data) => {
        try {
          const result = JSON.parse(data);
...
async function getapi(taskId, taskItemId) {
  return new Promise((resolve) => {
    $.post(
      api(
        `appid=china-joy&functionId=festival_floor_prod&body={"taskId":"${taskId}","taskItemId":"${taskItemId}","timestamp":${time},"skuId":"","taskType":null,"apiMapping":"/api/task/getReward"}&t=${timestamp}&loginType=2`
      ),
      async (error, response, data) => {
        //$.log(data)
        try {
...
async function index() {
  return new Promise((resolve) => {
    $.post(
      api(
        'appid=china-joy&functionId=festival_floor_prod&body={"apiMapping":"/api/index/index"}&t=1643262435851&loginType=2'
      ),
      async (error, response, data) => {
        //$.log(data)
        try {

...
let chetou_number = process.env.KOI_CHETOU_NUMBER
  ? Number(process.env.KOI_CHETOU_NUMBER)
  : 0;
...
if (!jdPandaToken && !RabbitUrl) {
  console.log(
    `请填写Panda获取的Token,变量是gua_cleancart_PandaToke 或者填写Rabbit获取的logurl，变量是Rabbit_Url`
  );
  return;
}
var logs;
...
!(async () => {
  ...
  console.log(
    `当前配置的车头数目：${chetou_number}，是否开启公平模式：${fair_mode}`
  );
    ...
    try {
      ...
      do {
        try {
          ...
          // 额外捕获异常
          console.error(
            `第${cnt}次请求第${cookieIndex} 个账号信息出现错误，错误为${error}，捕获该异常，3次后进行下一个账号`
          );
          cnt++;
        }
      } while (cnt < 3);
      if (help) {
        while (tools.length > 0 && remainingTryCount > 0) {
          ...
          console.debug(
            `尝试用 ${tool.id} 账号助力 ${help.id} 账号，用于互助的账号剩余 ${tools.length}`
          );
          let helpNum = 0;
          do {
            try {
              ...
              // 额外捕获异常
              console.error(
                `尝试用 ${tool.id} 账号助力 ${help.id} 出现错误，错误为${error}，捕获该异常，跳过此账号继续执行助力~`
              );
              helpNum++;
            }
          } while (helpNum < 5);
          ...
          if (help.assist_full) {
            console.info(
              `账号 ${help.id} 助力完成，累计获得 ${help.helpCount} 次互助，将尝试下一个账号`
            );
            break;
          }
        ...
        // 获取失败，跳过
        console.info(
          `账号 ${cookieIndex} 获取信息失败，具体原因见上一行，将尝试下一个账号`
        );
      }
      ...
      // 额外捕获异常
      console.error(
        `处理当前账号 ${cookieIndex} 时抛异常了，错误为${error}，捕获该异常，确保其他账号可以继续执行~`
      );
    }
  ...
  allMessage +=
    "上述就是本次的幸运锦鲤啦~ 自动开红包流程没出错的话，红包应该已经领到了~不过也可以手动前往 京东app/领券/锦鲤红包 去确认~\n";

  allMessage +=
    "（请以今日0点后第一次运行的消息为准。后续运行只是为了保底，避免第一次因各种未知异常而未完成运行）";
...
async function getHelpInfoForCk(cookieIndex, cookie) {
  ...
  let random = logs["random"].toString(),
  ...
  switch (data?.data?.result?.status) {
    case 1: //火爆
      console.debug(
        `h5launch 被风控，变成黑号了, data=${JSON.stringify(data)}`
      );
      return;
    case 2: //已经发起过
      break;
  ...
  if (data?.data?.result?.redpacketConfigFillRewardInfo) {
    ...
    console.info(
      `【京东账号${
        cookieIndex + 1
      }】 ${headmanNickName} 已获取红包 ${packetTotalSum}，剩余可拆红包为 ${calcCanTakeRedpacketCount(
        info
      )}`
    );
    ...
    for (
      let packetIdx = 0;
      packetIdx < info.redpacketConfigFillRewardInfo.length;
      packetIdx++
    ) {
      ...
      console.info(
        `红包 ${packetIdx + 1} 助力 ${packetInfo.hasAssistNum}/${
          packetInfo.requireAssistNum
        } ${status} ${packetInfo.packetAmount || "未开启"}/${
          packetInfo.operationWord
        }`
      );
    }
  }
  ...
  switch (data?.data?.code) {
    ...
    case 10003: //活动正在进行，火爆号
      console.debug(
        `h5activityIndex 被风控，变成黑号了, data=${JSON.stringify(data)}`
      );
      break;
...
async function appendRewardInfoToNotify(cookieIndex, cookie) {
  ...
  if (calcCanTakeRedpacketCount(data?.data?.result) > 0) {
    ...
    console.info(
      `【京东账号${
        cookieIndex + 1
      }】 ${headmanNickName} 剩余可拆红包为 ${canTakeCount} 个，将尝试领取`
    );
  ...
  if (data?.data?.result?.redpacketConfigFillRewardInfo) {
    ...
    allMessage += `【京东账号${
      cookieIndex + 1
    }】 ${headmanNickName} 已获取红包 ${packetTotalSum} 元，剩余可拆红包为 ${calcCanTakeRedpacketCount(
      info
    )} 个（如开红包流程顺利，这里应该永远是0）\n`;
    ...
    for (
      ...
      allMessage += `红包 ${packetIdx + 1} 助力 ${packetInfo.hasAssistNum}/${
        packetInfo.requireAssistNum
      } ${status} ${packetInfo.packetAmount || "未开启"}/${
        packetInfo.operationWord
      }\n`;
...
function calcCanTakeRedpacketCount(info) {
  ...
  for (
    ...
    if (
      packetInfo.hasAssistNum >= packetInfo.requireAssistNum &&
      !packetInfo.packetAmount
    ) {
      count++;
    }
  }
...
async function with_retry(
  ...
  for (let tryIdex = 1; tryIdex <= max_retry_times; tryIdex++) {
    ...
    console.error(
      `[${tryIdex}/${max_retry_times}] ${ctx} 请求时似乎出错了，有可能是网络波动，将等待 ${
        retry_interval / 1000
      } 秒，最多试 ${max_retry_times} 次\n`
    );
...
async function helpThisUser(help, tool) {
  ...
  // 实际发起请求
  await requestApi("jinli_h5assist", tool.cookie, body).then(async function (
    data
  ) {
    ...
    $.post(
      ...
      (_, resp, data) => {
        try {
          ...
          $.logErr("Error: ", e, resp);
          console.warn(
            `请求${functionId}失败，data=${JSON.stringify(
              data
            )}, e=${JSON.stringify(e)}`
          );
        } finally {

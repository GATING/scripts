...
async function jdsign() {
  try {
    console.log(`签到开始........`);
    await getInfo(
      "https://pro.m.jd.com/mall/active/3S28janPLYmtFxypu37AYAGgivfp/index.html"
    ); //拍拍二手签到
    await $.wait(2000);
    await getInfo(
...
async function getInfo(url) {
  return new Promise((resolve) => {
    $.get(
      ...
      async (err, resp, data) => {
        try {
          $.encryptProjectId = resp.body.match(
            /"encryptProjectId\\":\\"(.*?)\\"/
          )[1];
          $.encryptAssignmentId = resp.body.match(
            /"encryptAssignmentId\\":\\"(.*?)\\"/
          )[1];
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.subCode == "0" && data.rewardsInfo) {
                ...
                if (
                  data.rewardsInfo.successRewards["3"] &&
                  data.rewardsInfo.successRewards["3"].length != 0
                ) {
                  console.log(
                    `${data.rewardsInfo.successRewards["3"][0].rewardName},获得${data.rewardsInfo.successRewards["3"][0].quantity}京豆`
                  );
...
function taskUrl(functionId, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${functionId}&body=${encodeURI(
      JSON.stringify(body)
    )}&appid=babelh5&sign=11&t=${new Date().getTime()}`,
    headers: {
      Host: "api.m.jd.com",

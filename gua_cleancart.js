...
jdSignUrl = $.isNode()
  ? process.env.gua_cleancart_SignUrl
    ? process.env.gua_cleancart_SignUrl
    : `${jdSignUrl}`
...
cleancartRun = $.isNode()
  ? process.env.gua_cleancart_Run
    ? process.env.gua_cleancart_Run
    : `${cleancartRun}`
  : $.getdata("gua_cleancart_Run")
  ? $.getdata("gua_cleancart_Run")
  : `${cleancartRun}`;

cleancartProducts = $.isNode()
    ...
    if (typeof res == "object" && res) {
      if (res.resultCode == 0) {
        if (!res.clearCartInfo || !res.subTitle) {
          ...
          if (res.subTitle) {
            num =
              (res.subTitle.match(/共(\d+)件商品/).length > 0 &&
                res.subTitle.match(/共(\d+)件商品/)[1]) ||
              0;
            msg += res.subTitle + "\n";
          ...
          if (num != 0) {
              ...
              for (let s of a.groupDetails || []) {
                if (toSDS(s.name)) {
                  ...
                  operations.push({
                    ...
                    useUuid:
                      (typeof s.useUuid !== "undefined" && s.useUuid) || false,
                  });
                }
              }
              // }
            ...
            if (operations.length == 0) {
              ...
              let clearBody = `{"homeWishListUserFlag":"1","userType":"0","updateTag":false,"showPlusEntry":"2","hitNewUIStatus":"1","cvhv":"049591","cartuuid":"hjudwgohxzVu96krv/T6Hg==","operations":${$.toStr(
                operations,
                operations
              )},"adid":"","coord_type":"0"}`;
...
function jdApi(functionId, body) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(`/client.action?functionId=${functionId}`, body),
      async (err, resp, data) => {
        try {
          if (err) {

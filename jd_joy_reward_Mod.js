...
async function joyReward() {
  try {
    ...
    if (new Date().getMinutes() == 59) {
      console.log(
        `脚本现在时间 ${new Date().Format("yyyy-MM-dd hh:mm:ss | S")}`
      );
    ...
    for (let j = 0; j <= 14; j++) {
      ...
      console.log(
        `\n正在尝试第` +
          (j + 1) +
          `次执行:${new Date().Format("yyyy-MM-dd hh:mm:ss | S")} \n`
      );
      const data = $.getExchangeRewardsRes.data;
      if (llChange500) {
        for (let item of data[giftSaleInfos]) {
          if (item.giftType === "jd_bean") {
            ...
            if (salePrice && rewardNum == 500) {
              ...
              console.log(
                `开始兑换${rewardNum}京豆,时间 ${new Date().Format(
                  "yyyy-MM-dd hh:mm:ss | S"
                )}`
              );
              await exchange(saleInfoId, "pet");
              console.log(
                `结束兑换API后时间 ${new Date().Format(
                  "yyyy-MM-dd hh:mm:ss | S"
                )}`
              );
              if ($.exchangeRes && $.exchangeRes.success) {
                if ($.exchangeRes.errorCode === "buy_success") {
                  ...
                  if ($.isNode() && process.env.JD_JOY_REWARD_NOTIFY) {
                    $.ctrTemp =
                      `${process.env.JD_JOY_REWARD_NOTIFY}` === "false";
...
function getExchangeRewards() {
  return new Promise((resolve) => {
    ...
    const option = {
      url:
        `https://jdjoy.jd.com/common/gift/getBeanConfigs?reqSource=h5&invokeKey=q8DNJdpcfRQ69gIx` +
        $.validate,
      headers: {
        Host: "jdjoy.jd.com",
        Accept: "*/*",
    ...
    $.get(option, (err, resp, data) => {
      try {
        if (err) {
          ...
          console.log(
            `${$.name} getExchangeRewards API请求失败，请检查网路重试`
          );
        } else {
...
function exchange(saleInfoId, orderSource) {
  ...
  return new Promise((resolve) => {
    ...
    const option = {
      url:
        `https://jdjoy.jd.com/common/gift/new/exchange?reqSource=h5&invokeKey=q8DNJdpcfRQ69gIx` +
        $.validate,

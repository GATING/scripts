...
import USER_AGENT, { requireConfig, wait, o2s, post } from "./TS_USER_AGENTS";
...
!(async () => {
  ...
  for (let [index, value] of cookiesArr.entries()) {
    ...
    let encryptProjectId: string =
      res.data.result.activityBaseInfo.encryptProjectId;
    await wait(1000);

    // 任务
    res = await api("superBrandTaskList", {
      source: "card",
      ...
      if (new Date().getHours() === 20) {
        ...
        for (let i = 0; i < userStarNum; i++) {
          ...
          if (res.data.result?.rewardComponent?.beanList?.length) {
            console.log(
              "抽奖获得京豆：",
              res.data.result.rewardComponent.beanList[0].quantity
            );
  ...
  for (let [index, value] of cookiesArr.entries()) {
    ...
    for (let code of shareCodes) {
      if (full.includes(code.itemId)) continue;
...
async function api(fn: string, body: object) {
  return await post(
    `https://api.m.jd.com/?uuid=&client=wh5&appid=ProductZ4Brand&functionId=${fn}&t=${Date.now()}&body=${encodeURIComponent(
      JSON.stringify(body)
    )}`,
    "",
    {

...
RemainMessage +=
  "【东东农场】京东->我的->东东农场,完成可兑换无门槛红包,可用于任意商品\n";
...
async function showMsg() {
  ...
  if ($.JDtotalcash) {
    ReturnMessage += `【特价金币】${$.JDtotalcash}币(≈${(
      $.JDtotalcash / 10000
    ).toFixed(2)}元)\n`;
  }
    ...
    if ($.JingXiang) {
      if ($.isRealNameAuth)
        if (cookie.includes("app_open"))
          ReturnMessage =
            `【账号名称】${$.nickName || $.UserName}(wskey已实名)\n` +
            ReturnMessage;
        else
          ReturnMessage =
            `【账号名称】${$.nickName || $.UserName}(已实名)\n` + ReturnMessage;
      else if (cookie.includes("app_open"))
        ReturnMessage =
          `【账号名称】${$.nickName || $.UserName}(wskey未实名)\n` +
          ReturnMessage;
      else
        ReturnMessage =
          `【账号名称】${$.nickName || $.UserName}(未实名)\n` + ReturnMessage;
    } else {
      ReturnMessage =
        `【账号名称】${$.nickName || $.UserName}\n` + ReturnMessage;
    }
    if (TempBaipiao) {
  ...
  return new Promise(async (resolve) => {
    setTimeout(async () => {
      ...
      if (epsignurl) {
        ...
        strsign = await getSignfromNolan("jingBeanDetail", {
          pageSize: "20",
          page: "1",
        });
          ...
          if (err) {
            ...
            if (data) {
              ...
              if (data?.others?.jingBeanExpiringInfo?.detailList) {
                ...
                detailList.map((item) => {
                  strGuoqi += `【${item["eventMassage"]
                    .replace("即将过期京豆", "")
                    .replace("年", "-")
                    .replace("月", "-")
                    .replace("日", "")}】过期${item["amount"]}豆\n`;
                });
              }
            } else {
          ...
          if (data) {
            ...
            if ($.jsRed > 0) {
              if ($.jsRedExpire > 0)
                $.message += `【京喜特价】${
                  $.jsRed
                }(将过期${$.jsRedExpire.toFixed(2)})元(原极速版) \n`;
...
function taskcashUrl(functionId, body = {}) {
  ...
  let strurl =
    ...
    `${escape(
      JSON["stringify"](body)
    )}&appid=lite-android&client=android&uuid=` +
    struuid +
    ...
    $.get(opt, async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data.code == 200) {
            data =
              data.data.userOperateList.length !== 0
                ? new Date(
                    data.data.userOperateList[0].time
                  ).toLocaleDateString()
                : "";
          } else {
            console.log(data.msg);

...
[task_local]
#东东健康社区
13 1,18 * * * https://raw.githubusercontent.com/GATING/scripts/main/jd_health.js, tag=东东健康社区, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

=====================Loon================
[Script]
cron "13 1,18 * * *" script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_health.js, tag=东东健康社区

====================Surge================
东东健康社区 = type=cron,cronexp="13 1,18 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_health.js

============小火箭=========
东东健康社区 = type=cron,script-path=https://raw.githubusercontent.com/GATING/scripts/main/jd_health.js, cronexpr="13 1,18 * * *", timeout=3600, enable=true
 */
        ...
        try {
          if (safeGet(data)) {
            ...
            if (taskId === -1) {
              ...
              if (oc(() => data.data.result.taskVos)) {
                console.log(
                  `\n【京东账号${$.index}（${$.UserName}）的${
                    ...
                    () =>
                      data.data.result.taskVos[0].assistTaskDetailVo.taskToken
                  )}\n`
                );
              ...
              console.log(
                `${oc(
                  () => data.data.result.taskVos[0].taskName
                )}任务，完成次数：${oc(
                  () => data.data.result.taskVos[0].times
                )}/${oc(() => data.data.result.taskVos[0].maxTimes)}`
              );
              if (
                oc(() => data.data.result.taskVos[0].times) ===
                oc(() => data.data.result.taskVos[0].maxTimes)
              )
                return;
              await doTask(
                oc(
                  () =>
                    data.data.result.taskVos[0].shoppingActivityVos[0].taskToken
                ),
                22,
                1
              ); //领取任务
              await $.wait(
                1000 * (oc(() => data.data.result.taskVos[0].waitDuration) || 3)
              );
              await doTask(
                oc(
                  () =>
              ...
              for (let vo of nc(
                oc(() =>
                  data.data.result.taskVos.filter(
                    (vo) => ![19, 25, 15, 21].includes(vo.taskType)
                  )
                ),
                []
              )) {
  ...
  return new Promise(async (resolve) => {
    ...
    $.post(options, async (err, resp, data) => {
      try {
        if (safeGet(data)) {
          data = $.toObj(data);
          let beans = data.data.result.jBeans.filter(

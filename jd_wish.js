      ...
      for (let v = 0; v < appIdArr.length; v++) {
        ...
        for (let j = 0; j < $.shareCode.length && $.canHelp; j++) {
          if ($.shareCode[j].appId === appId) {
            console.log(
              `${$.UserName} 去助力 ${$.shareCode[j].use} 的助力码 ${$.shareCode[j].code}`
            );
...
async function jd_wish() {
  try {
    ...
    let getHomeDataRes = (await healthyDay_getHomeData(false)).data.result
      .userInfo;
    let forNum = Math.floor(
      getHomeDataRes.userScore / getHomeDataRes.scorePerLottery
    );
    await $.wait(2000);
...
async function healthyDay_getHomeData(type = true) {
  return new Promise(async (resolve) => {
    $.post(
      taskUrl("healthyDay_getHomeData", {
        appId: appId,
        taskToken: "",
        channelId: 1,
      }),
                    ...
                    if (vo.taskType === 13 || vo.taskType === 12) {
                      ...
                      for (let key of Object.keys(vo.followShopVo)) {
                        ...
                        if (followShopVo.status !== 2) {
                          console.log(
                            `【${followShopVo.shopName}】${vo.subTitleName}`
                          );
                          await harmony_collectScore({
                            appId: appId,
                      ...
                      for (let key of Object.keys(vo.productInfoVos)) {
                        ...
                        if (productInfoVos.status !== 2) {
                          console.log(
                            `【${productInfoVos.skuName}】${vo.subTitleName}`
                          );
                          await harmony_collectScore({
                            appId: appId,
                      ...
                      for (let key of Object.keys(vo.shoppingActivityVos)) {
                        ...
                        if (shoppingActivityVos.status !== 2) {
                          console.log(
                            `【${shoppingActivityVos.title}】${vo.subTitleName}`
                          );
                          if (vo.taskType === 9) {
                            await harmony_collectScore({
                              appId: appId,
                      ...
                      console.log(
                        `【京东账号${$.index}（${$.UserName}）的${appName}好友互助码】${vo.assistTaskDetailVo.taskToken}\n`
                      );
          ...
          if (safeGet(data)) {
            ...
            if (data && data.data && data.data.bizCode === 0) {
              if (taskType === 13) {
                ...
                console.log(
                  `助力成功：您的好友获得${data.data.result.score}金币\n`
                );
              } else {
              ...
              if (taskType === 13) {
                ...
                if (
                  data.code === -30001 ||
                  (data.data && data.data.bizCode === 108)
                )
                  $.canHelp = false;
...
function interact_template_getLotteryResult() {
  return new Promise((resolve) => {
    $.post(
      taskUrl("interact_template_getLotteryResult", { appId: appId }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} getLotteryResul API请求失败，请检查网路重试`
            );
          } else {
            if (safeGet(data)) {
              data = JSON.parse(data);
              let userAwardsCacheDto =
                data &&
                data.data &&
                data.data.result &&
              ...
              if (userAwardsCacheDto) {
                if (userAwardsCacheDto.type === 2) {
                  console.log(
                    `抽中：${userAwardsCacheDto.jBeanAwardVo.quantity}${
                      userAwardsCacheDto.jBeanAwardVo.ext || `京豆`
                    }`
                  );
...
function taskUrl(function_id, body = {}) {
  return {
    ...
    headers: {
      ...
      Referer:
        "https://h5.m.jd.com/babelDiy/Zeus/4FdmTJQNah9oDJyQN8NggvRi1nEY/index.html",

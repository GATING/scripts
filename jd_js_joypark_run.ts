...
!(async () => {
  ...
  for (let [index, value] of cookiesArr.entries()) {
    ...
    try {
      ...
      if (
        res.data.runningCashStatus.currentEndTime &&
        res.data.runningCashStatus.status === 0
      ) {
        console.log("可提现", rewardAmount);
        res = await api("runningPrizeDraw", {
          linkId: "L-sOanK_5RJCz7I314FpnQ",
          type: 2,
        });
        await wait(2000);
...
async function api(fn: string, body: object) {
  ...
  let params: string = `functionId=${fn}&body=${JSON.stringify(
    body
  )}&t=${timestamp}&appid=activities_platform&client=ios&clientVersion=3.1.0&cthr=1`;
...
async function runningPageHome() {
  return get(
    `https://api.m.jd.com/?functionId=runningPageHome&body=%7B%22linkId%22:%22L-sOanK_5RJCz7I314FpnQ%22,%22isFromJoyPark%22:true,%22joyLinkId%22:%22LsQNxL7iWDlXUs6cFl-AAg%22%7D&t=${Date.now()}&appid=activities_platform&client=ios&clientVersion=3.1.0`,
    {
      Host: "api.m.jd.com",
      Origin: "https://h5platform.jd.com",
      "User-Agent": "jdltapp;",
...
async function team(fn: string, body: object) {
  ...
  return await get(
    `https://api.m.jd.com/?functionId=${fn}&body=${encodeURIComponent(
      JSON.stringify(body)
    )}&t=${timestamp}&appid=activities_platform&client=ios&clientVersion=3.1.0&cthr=1&h5st=${h5st}`,
    {
      Host: "api.m.jd.com",
      "User-Agent": "jdltapp;",
      Origin: "https://h5platform.jd.com",

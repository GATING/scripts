...
!(async () => {
  console.log(
    "入口下拉：https://prodev.m.jd.com/mall/active/3z1Vesrhx3GCCcBn2HgbFR4Jq68o/index.html"
  );
  if (!cookiesArr[0]) {
    $.msg(
      $.name,
    ...
    $.get(
      {
        ...
        headers: {
          ...
          Referer:
            "https://prodev.m.jd.com/mall/active/2Rkjx8aT5eKaQnUzn8dwcR6jNanj/index.html",
...
function join() {
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `https://jdjoy.jd.com/module/task/draw/join?configCode=${
          $.configCode
        }&fp=${randomWord(false, 32, 32)}&eid=`,
        headers: {
          Host: "jdjoy.jd.com",
          accept: "*/*",
          "content-type": "application/json",
...
function doTask(taskType, itemId, taskid) {
  return new Promise((resolve) => {
    let options = taskPostUrl(
      "doTask",
      `{"configCode":"${$.configCode}","taskType":${taskType},"itemId":"${itemId}","taskId":${taskid}}`
    );

...
function getActInfo() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("assignmentList", {}, "appid=jwsp"),
      (err, resp, data) => {
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 200) {
                $.encryptProjectId =
                  data.result.assignmentResult.encryptProjectId;
                console.log(
                  `活动名称：${data.result.assignmentResult.projectName}`
                );
...
function getUserInfo(info = true) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("homePageV2", {}, "appid=SecKill2020"),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err},${jsonParse(resp.body)["message"]}`);
...
function getTaskList() {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("queryInteractiveInfo", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${err},${jsonParse(resp.body)["message"]}`);
                  ...
                  if (vo["completionCnt"] < vo["assignmentTimesLimit"]) {
                    if (vo["assignmentType"] === 1) {
                      if (vo["ext"][vo["ext"]["extraType"]].length === 0)
                        continue;
                      for (
                        let i = vo["completionCnt"];
                        i < vo["assignmentTimesLimit"];
                        ++i
                      ) {
                        console.log(
                          `去做${vo["assignmentName"]}任务：${i + 1}/${
                            vo["assignmentTimesLimit"]
                          }`
                        );
                        let body = {
                          encryptAssignmentId: vo["encryptAssignmentId"],
                          itemId:
                            vo["ext"][vo["ext"]["extraType"]][i]["itemId"],
                          actionType: 1,
                          completionFlag: "",
                        };
                        await doTask(body);
...
async function readpacksign() {
  ...
  return new Promise((resolve) => {
    body =
      'uuid=16488990475261100141756.111.1650014649515&clientVersion=10.3.0&client=wh5&osVersion=&area=4_48201_54794_0&networkType=unknown&functionId=signRedPackage&body={"random":`${random}`,"log":`${log}`,"sceneid":"MShPageh5","ext":{"platform":"1","eid":"","referUrl":-1,"userAgent":-1}}&appid=SecKill2020';
...
function getJdLogs(url) {
  return new Promise((resolve) => {
    const options = {
      url: `${url}?${new Date()}&logNums=${logNums}`,
      timeout: 10000,
      headers: {
        "User-Agent":

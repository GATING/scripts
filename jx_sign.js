    ...
    if ($.canHelp) {
      if ($.shareCodes && $.shareCodes.length) {
        ...
        for (let j = 0; j < $.shareCodes.length; j++) {
          ...
          console.log(
            `账号 ${$.UserName} 去助力 ${$.shareCodes[j].use} 的互助码 ${$.shareCodes[j].smp}`
          );
          if ($.shareCodes[j].max) {
...
function helpSignhb(smp = "") {
  return new Promise((resolve) => {
    $.get(
      taskUrl(
        "signhb/query",
        `type=1&signhb_source=${$.signhb_source}&smp=${smp}&ispp=1&tk=`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
      ...
      try {
        if (err) {
          ...
          if (data.ret === 0) {
            console.log(
              $.signhb_source === "5"
                ? `完成任务 获得${data.sendxd}喜豆`
                : `完成任务 获得${data.sendhb}红包`
            );
      ...
      try {
        if (err) {
          ...
          if (data.ret === 0) {
            console.log(
              $.signhb_source === "5"
                ? `开启宝箱 获得${data.sendxd}喜豆`
                : `开启宝箱 获得${data.sendhb}红包`
            );
          } else {
            console.log(JSON.stringify(data));
          }
...
function SignedInfo() {
  return new Promise((resolve) => {
    $.get(
      JDtaskUrl("SignedInfo", `_=${Date.now()}`),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(JSON.stringify(err));
...
function JDtaskUrl(functionId, body = "") {
  let url = `https://wq.jd.com/jxjdsignin/${functionId}?${
    body ? `${body}&` : ""
